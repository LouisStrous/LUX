...
static char rcsid[] __attribute__ ((unused)) =
 "$Id: fun4.c,v 4.0 2001/02/07 20:37:01 strous Exp $";
...
*/
{
 ...
 if (array_type(ps[1]) > type) {
   ...
   p1.l = array_data(ana_converts[type](1, &ps[0]));
   p2.l = array_data(ps[1]);
 } else {
   p1.l = array_data(ps[0]);
   p2.l = array_data(ana_converts[type](1, &ps[1]));
 }
 dstype = type;

 /* we convert <gx> and <gy> to ANA_LONG */
 gx = array_data(ana_long(1, &ps[2]));
 gy = array_data(ana_long(1, &ps[3]));
 ...
 result_sym = array_scratch(ANA_FLOAT, 3, dim);
   ...
   DELTA is a displacement grid as generated by GRIDMATCH */
{
  ...
  iq = ana_float(1, &ps[1]);	/* ensure that it is FLOAT */
...
/* Fixed overflow problem.  LS 19nov99 */
{
  ...
  for (i = 0; i < nFac; i++) 	/* check that factors are legal */
    if (factors[i] < 1)
      return anaerror("Compression factors must be positive integers", ps[1]);

  if (narg >= 3) {		/* have <axes> */
    iq = ps[1];
    /* number of factors must equal to number of axes */
    if ((nFac == 1
	 && symbol_class(iq) != ANA_SCALAR
	 && (symbol_class(iq) != ANA_ARRAY || array_size(iq) != 1))
	|| (nFac > 1
	    && symbol_class(iq) != ANA_ARRAY
	    && array_size(iq) != nFac))
      return cerror(INCMP_ARG, ps[1]);
    allAxes = 0;
    ...
    if (nFac == 1) {		/* compress all dimensions */
      ...
      iq = ana_indgen(1, &ps[1]); /* default: INDGEN(<factors>) */
      allAxes = 0;
    }
  }
  ...
  for (i = 0; i < srcinfo.ndim; i++)
    if (div[i] > srcinfo.dims[i])
      return anaerror("Compression factor %1d exceeds the %1d elements in dimension %1d", (narg > 3 && ps[2])? ps[2]: 0, div[i], srcinfo.dims[i], i);
  ...
  zerobytes(trgt.b, array_size(iq)*ana_type_size[array_type(iq)]);
...
/*sort contents of a copy of an array */
{
  ...
  if (!symbolIsArray(iq))
    return cerror(NEED_ARR, iq);
  type = array_type(iq);
  if (type >= ANA_CFLOAT || type == ANA_TEMP_STRING || type == ANA_LSTRING)
    return cerror(ILL_TYPE, iq, typeName(type));
  if (type == ANA_STRING_ARRAY)
    type = ANA_DOUBLE + 1;	/* so we get the right sort function */
  result_sym = array_clone(iq, type);
  ...
  if (ana_replace(result_sym, iq) != 1)
    return cerror(IMPOSSIBLE, 0);
  ...
  switch (internalMode & ~4) {
    default:
      anaerror("Cannot select multiple sort methods!  Default is used.", 0);
 ...
 *ny2 = ms;			/* return output height */
 ...
 if (sym_flag) {
   result_sym = array_scratch(ANA_BYTE, 2, dim);
   if (result_sym < 0)
     return ANA_ERROR;
 } else {
   result_sym = findVarName(zoomtemp, 0);
   if (redef_array(result_sym, ANA_BYTE, 2, dim) != 1)
     return ANA_ERROR;
 }
 *symout = result_sym;
 ...
 when it is used only for a shift */
{
  ...
  nd = array_num_dims(iq);
  if (nd > 2)
    return anaerror("Need 1D or 2D array", iq);
  nx = array_dims(iq)[0];
  if (nd == 2)
    ny = array_dims(iq)[1];
  else
    ny = 1;
  if (nx < 2)
    return anaerror("Need at least 2 elements in 1st dimension", iq);
  n = array_size(iq);
  nb = n*ana_type_size[regrid_type];		/* # of bytes */
  /* get the shifts */
  if (float_arg_stat(ps[1], &dx) != ANA_OK)
    return ANA_ERROR;
  dy = 0.0;
  if (narg > 2 && float_arg_stat(ps[2], &dy) != ANA_OK)
    return ANA_ERROR;
...
/* REGRIDLS(<data>,<gx>,<gy>,<nx>,<ny>) */
{
  ...
  type = array_type(ps[0]);
  gridx = array_data(ana_float(1, &ps[1]));
  gridy = array_data(ana_float(1, &ps[2]));
  nx = int_arg(ps[3]);
  ny = int_arg(ps[4]);
...
/* y = CTOP(x [, x0, y0]) */
{
  ...
  if (type != ANA_FLOAT && type != ANA_DOUBLE)
    return anaerror("CTOP() works only on FLOAT or DOUBLE arrays", ps[0]);
...
/* y = PTOC(x [, nx, ny, x0, y0]) */
{
  ...
  if (type != ANA_FLOAT && type != ANA_DOUBLE)
    return anaerror("PTOC() works only on FLOAT or DOUBLE arrays", ps[0]);
