...
/// advance(i0);
/// \endverbatim
int32_t lux_ivarl_copy_eachaxis_(ArgumentCount narg, Symbol ps[],
...
// axis separately
{
  ...
  case 2:                       // source, axes
    ...
    axes = ptrs[1].i32;
    naxes = infos[1].nelem;
    if (infos[0].setAxes(infos[1].nelem, ptrs[1].i32,
                         SL_EACHROW | SL_UNIQUEAXES) < 0)
      return LUX_ERROR;
    if (isfunction) {
      iret = 2;
      if (infos[iret].setAxes(infos[1].nelem, ptrs[1].i32,
                              SL_EACHROW | SL_UNIQUEAXES) < 0)
        return LUX_ERROR;
    } else
      iret = 0;
                          ...
                          double (*f)(double, double, double))
{
  ...
  case LUX_INT32:
    while (infos[1].nelem--)
      *ptrs[3].d++ = f((double) *ptrs[0].i32++, *ptrs[1].d++, *ptrs[2].d++);
    break;
  case LUX_INT64:
    while (infos[1].nelem--)
                                         ...
                                         double, double))
{
  ...
  case LUX_INT32:
    while (infos[0].nelem--)
      *ptrs[5].d++ = f((double) *ptrs[0].i32++, 0.0, *ptrs[1].d, *ptrs[2].d,
                       *ptrs[3].d, *ptrs[4].d);
    break;
  case LUX_INT64:
    while (infos[0].nelem--)
...
lux_d_dd_iarq_0z_1_f_(ArgumentCount narg, Symbol ps[], double (*f)(double, double))
{
  ...
  case LUX_INT32:
    while (infos[1].nelem--)
      *ptrs[1].d++ = f((double) *ptrs[0].i32++, 0.0);
    break;
  case LUX_INT64:
    while (infos[1].nelem--)
                                                ...
                                                double [2][3]))
{
  ...
  case LUX_INT32:
    while (infos[0].nelem--) {
      f((double) *ptrs[0].i32++, 0.0, pvh, pvb);
      pvh += 2;
      pvb += 2;
    }
    break;
                                             ...
                                             double [2][3]))
{
  ...
  while (infos[0].nelem--) {
    f(*ptrs[0].d++, 0.0, *ptrs[1].i32, (double (*)[3]) ptrs[2].d);
    ptrs[2].d += 6;
  }
  return sa.result();
}
                                             ...
                                             int32_t *, int32_t *, double *))
{
  ...
  case LUX_INT32:
    while (infos[0].nelem--) {
      if (f((double) *ptrs[0].i32++, 0.0, &y, &m, &d, &fd)) {
        ptrs[1].d[0] = 0.0;
        ptrs[1].d[1] = 0.0;
        ptrs[1].d[2] = 0.0;
      } else {
                                          ...
                                          int32_t, double *))
{
  ...
  while (nrepeat--) {
    f(ptrs[0].d, infos[0].rdims[0], *ptrs[1].i32, *ptrs[2].i32, ptrs[3].d);
                           ...
                           int32_t (*f)(double *, int32_t, int32_t, double *))
{
  ...
  size_t nrepeat = infos[0].nelem/infos[0].rdims[0];
  while (nrepeat--) {
    f(ptrs[0].d, infos[0].rdims[0], *ptrs[1].i32, ptrs[2].d);
    ptrs[0].d += infos[0].rdims[0];
                                           ...
                                           double [3]))
{
  ...
  size_t nelem = infos[0].nelem/3;
  while (nelem--) {
    f(*ptrs[1].i32, ptrs[0].d[0], ptrs[0].d[1], ptrs[0].d[2], ptrs[2].d);
    ptrs[0].d += 3;
    ptrs[2].d += 3;
  }
  return sa.result();
                                             ...
                                             double *, double *))
{
  ...
  while (nelem--) {
    f(*ptrs[0].i32, ptrs[1].d, &ptrs[2].d[0], &ptrs[2].d[1], &ptrs[2].d[2]);
    ptrs[1].d += 3;
    ptrs[2].d += 3;
  }
  return sa.result();
                          ...
                          int32_t (*f)(int32_t, double *, double *))
{
  ...
  if (sa.result() < 0)
    return LUX_ERROR;
  while (infos[0].nelem--) {
    f(*ptrs[0].i32++, &ptrs[1].d[0], &ptrs[1].d[1]);
    ptrs[1].d += 2;
  }
  return sa.result();
}
                                             ...
                                             size_t tgtcount, size_t tgtstride))
{
  ...
  case 4:                       // source, axis, param1, param2
    ...
    ipar2 = 3;
    iret = 4;
    infos[0].setAxes(infos[1].nelem, ptrs[1].i32, SL_EACHROW);
    infos[iret].setAxes(infos[1].nelem, ptrs[1].i32, SL_EACHROW);
    break;
  }
  do {
                                                 ...
                                                 double [3][3], double [3][3]))
{
  ...
  case LUX_INT32:
    while (infos[0].nelem--) {
      f((double) *ptrs[0].i32++, 0.0, (double (*)[3]) ptrs[1].d,
