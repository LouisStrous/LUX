...
while (<$ifh>) {
  $line .= $_;
  if ($line =~ s/.*?\nSymbol\s+(?<id>\w+)\(.*?,.*?, *(?<decl>.*?)\);//s) {
    my $id = clean($+{id});
    my $decl = clean($+{decl});
    if (exists $expected_types{$id}) {
      die "Duplicate declaration of $id with $decl in bindings.hh line $."
        if $expected_types{$id} eq $decl;
      die "Conflicting declaration of $id:\n"
        . " old: $expected_types{$id}\n"
        . " new: $decl";
    }
    $expected_types{$id} = $decl;
  }
}
...
foreach my $file (@files) {
  ...
  while (<$ifh>) {
    ...
    if (/^BIND\((.*?)\)/) {
      ...
      if (not $declare) {
        print "$file:$.: $f NOT FOUND (binding.pl)\n";
        next;
      }
      $declare =~ s/\(.*?\)/$data[0]/;
      print $ofh "#if $data[7]\n" if $data[7];
      ...
      //
      // This glue function was generated by bindings.pl based on $file
      // line $. and may be overwritten at the next compilation.
      Symbol
      lux_$data[0]_$data[2](int32_t narg, int32_t ps[])
      {
        $declare;
