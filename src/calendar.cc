 ...
 */
static void CJDtoCal(double CJD, int32_t *year, int32_t *month, double *day,
                     void (*f)(int32_t, int32_t *, int32_t *, int32_t *))
{
  // separate the CJD into a CJDN and a day fraction
  int32_t CJDN = floor(CJD);
 ...
 */
static double CaltoCJD(int32_t year, int32_t month, double day,
                       int32_t (*f)(int32_t, int32_t, int32_t))
{
  int32_t d = floor(day);
  double part = day - d;
  int32_t jd = f(year, month, d);
 ...
 */
static void CJDNto3SA(int32_t const *CJDN, char **date,
                      void (*CJDNtoCal3A) (int32_t const *, int32_t *),
                      char const * const *monthnames)
{
  int32_t datec[3];
...
static char *CJDto3S(double CJD,
                     void (*CJDtoCal3)(double, int32_t *, int32_t *, double *),
                     char const * const *monthnames)
{
  int32_t year, month;
  double day;
 ...
 */
static int32_t stralnumtouppercmp(char const * const a, char const * const b)
{
  char const *pa, *pb;
 ...
 */
static size_t find_name(char * const name, int32_t num_names,
 ...
 */
static int32_t S3toCJDN(char const * date,
                    int32_t (*Cal3toCJDN) (int32_t, int32_t, int32_t),
...
/** Translates a calendar date in text to a Chronological Julian Day
    ...
    no legal date was recognized in the input.
 */
static void S3toCJDNA(char * const * date, int32_t *CJDN,
                      void (*Cal3toCJDNA) (int32_t const *, int32_t *),
                      size_t nmonthnames, char const * const *monthnames)
{
  int32_t datec[3];
  size_t i, n;
...
static double S3toCJD(char const * date,
                      double (*Cal3toCJD) (int32_t, int32_t, double),
                      size_t nmonthnames, char const * const *monthnames)
{
  int32_t year, month;
  double day;
...
void CJDNtoGregorian(int32_t CJDN, int32_t *year, int32_t *month, int32_t *day)
{
  ...
  int32_t x1 = fquotient(5*y1 + 2, 153);
  int32_t c0 = fquotient(x1 + 2, 12);
  *year = 100*x3 + x2 + c0;
  *month = x1 - 12*c0 + 3;
  *day = y1 - fquotient(153*x1 - 3, 5);
}
//--------------------------------------------------------------------------
...
{
  *CJD = GregoriantoCJD((int32_t) date[0], (int32_t) date[1], date[2]);
}
//--------------------------------------------------------------------------
...
{
  *CJD = JuliantoCJD((int32_t) date[0], (int32_t) date[1], date[2]);
}
//--------------------------------------------------------------------------
...
{
  *CJD = CommontoCJD((int32_t) date[0], (int32_t) date[1], date[2]);
}
//--------------------------------------------------------------------------
...
/* The Hebrew calendar year begins with day 1 of month 7 (= New Year)
   ...
   day begin at 0. */
//--------------------------------------------------------------------------
void CJDNtoHebrew(int32_t CJDN, int32_t *year, int32_t *month, int *day)
{
  int32_t Hebrew_cycm2rd(int32_t x1, int32_t x3);

  /* running day number since day 1 of month 1 preceding New Year of
     year 1 */
  // -2147483647 ≤ CJDN ≤ 2147483647
  int32_t y4 = CJDN - HEBREW_EPOCH + 177;
  ...
  auto d1 = fdivide(y4, 1447);
  int32_t y1 = 49*d1.quot + (23*d1.quot + 25920*d1.rem + 13835)/765433;
  /* -2147135826 ≤ CJDN ≤ 2147483647
     -2147483647 ≤ y₄ ≤ 2147135826
     -72720640 ≤ y₁ ≤ 72708859 */
  ...
  // provisional calculation year number
  int32_t x1 = fquotient(19*y1 + 17, 235);
  // -5879541 ≤ x₁ ≤ 5878588
  int32_t x3 = y1 - fquotient(235*x1 + 1, 19); /* provisional calculation
                                           month number */
  ...
  if (z4 < 0 || z4 > 28) {        // the month number may be wrong
    if (z4 < 0) {         // the provisional month was one too great
      --y1;
      x1 = fquotient(19*y1 + 17, 235);
      x3 = y1 - fquotient(235*x1 + 1, 19);
      c4 = Hebrew_cycm2rd(x1, x3);
      z4 = y4 - c4;
    } else {             // the provisional month may be one too small
      int32_t y1b = y1 + 1;
      int32_t x1b = fquotient(19*y1b + 17, 235);
      int32_t x3b = y1b - fquotient(235*x1b + 1, 19);
   ...
   not checked.  LS 24dec2012 */
{
  ...
  int32_t c1 = fquotient(235*x1 + 1, 19);
  ...
  /* calculate the number of days between 1 Tishri A.M. 1 and the
     ...
     http://http://aa.quae.nl/en/reken/juliaansedag.html */
  auto d = fdivide(c1, 1095);
  int32_t nu1 = 32336*d.quot
    + fquotient(15*d.quot + 765433*d.rem + 12084, 25920);
  ...
  /* calculate the number of days between 1 Tishri A.M. 1 and the
     first day of running month c₁, taking into account the first
     and second delays. */
  int32_t nu2 = nu1 + fquotient(6*fremainder(nu1, 7), 7)%2;
...
//--------------------------------------------------------------------------
int32_t HebrewtoCJDN(int year, int month, int day)
   ...
   -5879540 and 5878588, inclusive */
{
  ...
  /* Calculate the running day number of the first day of calculation
     ...
     day 7) of year A.M. 1 */
  int32_t rd = Hebrew_cycm2rd(x1, x3);
...
{
  *CJD = HebrewtoCJD((int32_t) date[0], (int32_t) date[1], date[2]);
}
//--------------------------------------------------------------------------
...
{
  *CJD = IslamictoCJD((int32_t) date[0], (int32_t) date[1], date[2]);
}
//--------------------------------------------------------------------------
...
void CJDNtoEgyptian(int32_t CJDN, int32_t *year, int32_t *month, int32_t *day)
{
  /* This algorithm by Louis Strous
     (http://aa.quae.nl/en/reken/juliaansedag.html) */
  // era of Nabonassar
  int32_t y2 = CJDN - EGYPTIAN_EPOCH;
  auto d = fdivide(y2, 365);
  *year = d.quot + 1;
  d = fdivide(d.rem, 30);
  *month = d.quot + 1;
  *day = d.rem + 1;
...
{
  *CJD = EgyptiantoCJD((int32_t) date[0], (int32_t) date[1], date[2]);
}
//--------------------------------------------------------------------------
...
{
  *trecena = fremainder(CJDN + 5, 13) + 1;
  *venteina = fremainder(CJDN + 16, 20) + 1;
  auto d = fdivide(CJDN + 65, 365);
  *haab_day = fremainder(d.rem, 20);
  *haab_month = d.rem/20 + 1;
  *year_trecena = fremainder(d.quot + 5, 13) + 1;
  *year_venteina = fremainder(5*d.quot + 11, 20) + 1;
}
//--------------------------------------------------------------------------
...
{
  int32_t trecena, venteina, haab_day, haab_month, year_trecena, year_venteina;
  char *date;
...
void CJDtoMayan(double CJD, int32_t *trecena, int32_t *venteina,
                ...
                int32_t *year_trecena, int32_t *year_venteina)
{
  int32_t ihaab_day, CJDN;
  double frac;

  CJDN = (int32_t) floor(CJD);
  frac = CJD - CJDN;
...
{
  int32_t trecena, venteina, haab_month, year_trecena, year_venteina;
  double haab_day;
  char *date;
...
{
  int32_t do_tzolkin = (trecena && venteina);
  int32_t do_haab = (haab_month != 0);
  int32_t do_year = (year_trecena && year_venteina);
  int32_t haab, tzolkin, year, haab_year;

  if (do_tzolkin) {
    trecena = fremainder(trecena - 6, 13);
    venteina = fremainder(venteina - 17, 20);
    tzolkin = fremainder(40*trecena - 39*venteina, 260);
  }
  if (do_haab)
    haab = fremainder((haab_month - 4)*20 + haab_day - 5, 365);
  if (do_year) {
    if ((year_venteina - 12) % 5)
      return 0;                 // impossible year venteina
    year_trecena = fremainder(year_trecena - 6, 13);
    year_venteina = fremainder(fquotient(year_venteina - 12, 5), 4);
    year = fremainder(40*year_trecena + 13*year_venteina, 52);
  ...
  case 0:
    return 0;
  case 1:                       // tzolkin
    return CJDN_upper - fremainder(CJDN_upper - tzolkin, 260);
  case 2:                       // haab
    return CJDN_upper - fremainder(CJDN_upper - haab, 365);
  case 3:                       // tzolkin & haab
    {
      int32_t tzolkin_haab = fremainder(365*tzolkin - 364*haab, 18980);
      return CJDN_upper - fremainder(CJDN_upper - tzolkin_haab, 18980);
    }
  case 4:                       // year
    return CJDN_upper - fremainder(CJDN_upper - haab_year, 18980);
  case 5:                       // tzolkin & year
    {
      int32_t CJDN_tzolkin = CJDN_upper - fremainder(CJDN_upper - tzolkin, 260);
      int32_t CJDN_haab_year = CJDN_upper
        - fremainder(CJDN_upper - haab_year, 18980);
      ...
      return CJDN_tzolkin
        + (18980-260)*fquotient(CJDN_tzolkin - CJDN_haab_year, 260);
    }
  case 6:                       // haab & year
    {
      int32_t CJDN_haab = CJDN_upper - fremainder(CJDN_upper - haab, 365);
      int32_t CJDN_haab_year = CJDN_upper
        - fremainder(CJDN_upper - haab_year, 18980);
      return CJDN_haab + (18980-365)*fquotient(CJDN_haab - CJDN_haab_year, 365);
    }
...
//--------------------------------------------------------------------------
static void MayanSto(char const *date, int32_t *CJDN, double *CJD)
{
  int32_t n, nc, datepos, inumber;
  double number, haab_day;
  char *name;
  size_t i;
  int32_t trecena, venteina = -1, haab_month = -1,
...
//--------------------------------------------------------------------------
double
MayantoCJD(int32_t trecena, int32_t venteina, double haab_day,
           int32_t haab_month, int32_t year_trecena,
           int32_t year_venteina, int32_t CJDN_upper)
{
  int32_t ihaab_day = floor(haab_day);
  double frac = haab_day - ihaab_day;
  int32_t CJDN = MayantoCJDN(trecena, venteina, ihaab_day, haab_month,
...
static int32_t longcount_periods[] = { 20, 20, 18, 20 };

void
CJDNtoLongCount(int32_t CJDN, int32_t *baktun, int32_t *katun,
                int32_t *tun, int32_t *uinal, int32_t *kin)
{
  int32_t d;

  d = CJDN - LONGCOUNT_EPOCH;
  auto x = fdivide(d, longcount_periods[3]);
  *kin = x.rem;
  x = fdivide(x.quot, longcount_periods[2]);
  *uinal = x.rem;
  x = fdivide(x.quot, longcount_periods[1]);
  *tun = x.rem;
  x = fdivide(x.quot, longcount_periods[0]);
  *baktun = x.quot;
  *katun = x.rem;
}
//--------------------------------------------------------------------------
void CJDNtoLongCountA(int32_t const *CJDN, int32_t *longcount)
{
  int32_t i;
  Div<int32_t> x;
  ...
  for (i = arraysize(longcount_periods) - 1; i >= 0; i--) {
    x = fdivide(x.quot, longcount_periods[i]);
    longcount[i + 1] = x.rem;
  }
  longcount[0] = x.quot;
}
...
{
  int32_t baktun, katun, tun, uinal, kin;
  char *date;
...
//--------------------------------------------------------------------------
void
CJDtoLongCount(double CJD, int32_t *baktun, int32_t *katun,
               int32_t *tun, int32_t *uinal, double *kin)
{
  int32_t d;
  double frac;
  Div<int32_t> x;
  ...
  frac = CJD - d;
  d -= LONGCOUNT_EPOCH;
  x = fdivide(d, longcount_periods[3]);
  *kin = x.rem + frac;
  x = fdivide(x.quot, longcount_periods[2]);
  *uinal = x.rem;
  x = fdivide(d, longcount_periods[1]);
  *tun = x.rem;
  x = fdivide(d, longcount_periods[0]);
  *baktun = x.quot;
  *katun = x.rem;
}
//--------------------------------------------------------------------------
void CJDtoLongCountA(double const *CJD, double *longcount)
{
  ...
  d -= LONGCOUNT_EPOCH;
  for (i = arraysize(longcount_periods) - 1; i >= 0; i--) {
    auto x = fdivide(d, longcount_periods[i]);
    d = x.quot;
    longcount[i + 1] = x.rem;
  }
  longcount[0] = d + frac;
...
//--------------------------------------------------------------------------
int32_t
LongCounttoCJDN(int32_t baktun, int32_t katun, int32_t tun,
...
//--------------------------------------------------------------------------
void LongCounttoCJDNA(int32_t const *longcount, int32_t *CJDN)
{
...
{
  int32_t datec[5] = { 0, 0, 0, 0, 0 }, i, value;
  char *p;
...
//--------------------------------------------------------------------------
double
LongCounttoCJD(int32_t baktun, int32_t katun, int32_t tun,
               int32_t uinal, double kin)
{
  int32_t ikin = floor(kin);
