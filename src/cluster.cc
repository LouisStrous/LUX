...
//----------------------------------------------------------------
int32_t lux_cluster(ArgumentCount narg, Symbol ps[])
...
// until the closest cluster center has been found.
{
  ...
  if (gotIndex && useIndex) {        // INDEX
    ...
    if (!gotSample                // no SAMPLE
      ...
      switch (indexType) {
        ...
        case LUX_INT32:
          for (i = 0; i < nClusters; i++)
            if (clusterNumber.i32[i] < 0 // index too small
                || clusterNumber.i32[i] >= nClusters) { // index too large
              printf("CLUSTER - illegal index #%1d (%1d), ignore all\n",
                     i, clusterNumber.i32[i]);
              useIndex = 0;
              break;
            }
          break;
        case LUX_INT64:
          for (i = 0; i < nClusters; i++)
            if (clusterNumber.i64[i] < 0 // index too small
                || clusterNumber.i64[i] >= nClusters) { // index too large
              printf("CLUSTER - illegal index #%1d (%1d), ignore all\n",
                     i, clusterNumber.i32[i]);
              useIndex = 0;
              break;
  ...
  if (useIndex                        // usable cluster numbers
    ...
    switch (indexType) {
      ...
      case LUX_INT32:
        for (i = 0; i < nSample; i++) {
          j = clusterOtoC[clusterNumber.i32[i]];
          clusterSize[j]++;
        }
        break;
      case LUX_INT64:
        for (i = 0; i < nSample; i++) {
          j = clusterOtoC[clusterNumber.i64[i]];
          clusterSize[j]++;
        }
        break;
    }
      ...
      if (useIndex)                // get old cluster number
        switch (indexType) {
          ...
          case LUX_INT16:
            ...
            break;
          case LUX_INT32:
            curO = *clusterNumber.i32;
            break;
          case LUX_INT64:
            curO = *clusterNumber.i64;
            break;
        }
      ...
      switch (indexType) {        // save cluster number
        ...
        case LUX_INT16:
          *clusterNumber.i16++ = newO;
          break;
        case LUX_INT32:
          *clusterNumber.i32++ = newO;
          break;
        case LUX_INT64:
          *clusterNumber.i64++ = newO;
          break;
      }
