...
//-------------------------------------------------------------------------
int32_t lux_setenv(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_getenv(ArgumentCount narg, Symbol ps[])
...
//---------------------------------------------------------------------------
int32_t lux_spawn(ArgumentCount narg, Symbol ps[])                // execute a shell command
...
//---------------------------------------------------------------------------
int32_t lux_spawn_f(ArgumentCount narg, Symbol ps[])                // execute a shell command
 /* uses the system call to execute a shell command, but may not be the
         shell you want! depends on local setup */
/* Headers:
   <stdlib.h>: system()
 */
{
  ...
  scalar_value(result).i32 = system(p);
  return result;
 }
//-------------------------------------------------------------------------
...
//-------------------------------------------------------------------------
int32_t lux_ulib(ArgumentCount narg, Symbol ps[])
 //set ulib path
/* Headers:
   <stdio.h>: printf()
   <stdlib.h>: free()
...
//-------------------------------------------------------------------------
int32_t lux_type(ArgumentCount narg, Symbol ps[])
 // (stdin) printing
{
  int32_t        type_ascii(int32_t, int32_t [], FILE *);
...
//-------------------------------------------------------------------------
int32_t lux_printf(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_printf_f(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_format_set(ArgumentCount narg, Symbol ps[])
  ...
  case LUX_ARRAY:
    ...
    case LUX_INT16:
      ...
      while (j--) {
        if (!j && (theFormat.flags & FMT_MIX2))
          theFormat.spec_char[1] = '\0';
        Sprintf_tok(curScrat, (intmax_t) *ptr.i16++);
        fprintw(fp, curScrat);
        if (j && (internalMode & 4))
          nextformat(NULL, 1);
      }
    ...
    case LUX_INT32:
      ...
      while (j--) {
        if (!j && (theFormat.flags & FMT_MIX2))
          theFormat.spec_char[1] = '\0';
        Sprintf_tok(curScrat, (intmax_t) *ptr.i32++);
        fprintw(fp, curScrat);
        if (j && (internalMode & 4))
          nextformat(NULL, 1);
      }
    ...
    case LUX_INT64:
      ...
      while (j--) {
        if (!j && (theFormat.flags & FMT_MIX2))
          theFormat.spec_char[1] = '\0';
        Sprintf_tok(curScrat, (intmax_t) *ptr.i64++);
        fprintw(fp, curScrat);
        if (j && (internalMode & 4))
          nextformat(NULL, 1);
      }
...
//-------------------------------------------------------------------------
int32_t type_ascii(ArgumentCount narg, Symbol ps[], FILE *fp)
...
//-------------------------------------------------------------------------
int32_t type_formatted_ascii(ArgumentCount narg, Symbol ps[], FILE *fp)
  ...
  while (narg || theFormat.type == FMT_PLAIN) {
    if (theFormat.type == FMT_PLAIN) { // literal string
      ...
      switch (symbol_class(iq)) {
        ...
        case LUX_SCALAR:
          n = 1;
          p.i32 = &scalar_value(iq).i32;
          break;
        case LUX_ARRAY:
          n = array_size(iq);
          p.i32 = (int32_t*) array_data(iq);
          break;
        case LUX_CSCALAR:
          n = 1;
        ...
        switch (theFormat.type) {
          case FMT_INTEGER:
            switch (symbol_type(iq)) {
              ...
              case LUX_INT16:
                if (fp)
                  while (n--) {
                    fprintf(fp, thefmt, (int32_t) *p.i16++);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2)))
                      fputs(theFormat.plain, fp);
                  }
                else
                  while (n--) {
                    sprintf(curScrat, thefmt, (int32_t) *p.i16++);
                    curScrat += strlen(curScrat);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2))) {
                      strcpy(curScrat, theFormat.plain);
                      curScrat += strlen(curScrat);
              ...
              case LUX_INT32:
                if (fp)
                  while (n--) {
                    fprintf(fp, thefmt, (int32_t) *p.i32++);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2)))
                      fputs(theFormat.plain, fp);
                  }
                else
                  while (n--) {
                    sprintf(curScrat, thefmt, (int32_t) *p.i32++);
                    curScrat += strlen(curScrat);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2))) {
                      strcpy(curScrat, theFormat.plain);
                      curScrat += strlen(curScrat);
              ...
              case LUX_INT64:
                if (fp)
                  while (n--) {
                    fprintf(fp, thefmt, *p.i64++);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2)))
                      fputs(theFormat.plain, fp);
                  }
                else
                  while (n--) {
                    sprintf(curScrat, thefmt, *p.i64++);
                    curScrat += strlen(curScrat);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2))) {
                      strcpy(curScrat, theFormat.plain);
                      curScrat += strlen(curScrat);
          ...
          case FMT_FLOAT:
            switch (symbol_type(iq)) {
              ...
              case LUX_INT16:
                if (fp)
                  while (n--) {
                    fprintf(fp, thefmt, (double) *p.i16++);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2)))
                      fputs(theFormat.plain, fp);
                  }
                else
                  while (n--) {
                    sprintf(curScrat, thefmt, (double) *p.i16++);
                    curScrat += strlen(curScrat);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2))) {
                      strcpy(curScrat, theFormat.plain);
                      curScrat += strlen(curScrat);
              ...
              case LUX_INT32:
                if (fp)
                  while (n--) {
                    fprintf(fp, thefmt, (double) *p.i32++);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2)))
                      fputs(theFormat.plain, fp);
                  }
                else
                  while (n--) {
                    sprintf(curScrat, thefmt, (double) *p.i32++);
                    curScrat += strlen(curScrat);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2))) {
                      strcpy(curScrat, theFormat.plain);
                      curScrat += strlen(curScrat);
              ...
              case LUX_INT64:
                if (fp)
                  while (n--) {
                    fprintf(fp, thefmt, (double) *p.i64++);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2)))
                      fputs(theFormat.plain, fp);
                  }
                else
                  while (n--) {
                    sprintf(curScrat, thefmt, (double) *p.i64++);
                    curScrat += strlen(curScrat);
                    if (haveTrailer && (n || !(theFormat.flags & FMT_MIX2))) {
                      strcpy(curScrat, theFormat.plain);
                      curScrat += strlen(curScrat);
          ...
          case FMT_COMPLEX:
            switch (symbol_type(iq)) {
              ...
              case LUX_INT16:
                while (n--) {
                  Sprintf(curScrat, thefmt, (double) *p.i16++, (double) 0.0);
                  if (fp)
                    fputs(curScrat, fp);
                  else
                    curScrat += strlen(curScrat);
              ...
              case LUX_INT32:
                while (n--) {
                  Sprintf(curScrat, thefmt, (double) *p.i32++, (double) 0.0);
                  if (fp)
                    fputs(curScrat, fp);
                  else
                    curScrat += strlen(curScrat);
              ...
              case LUX_INT64:
                while (n--) {
                  Sprintf(curScrat, thefmt, (double) *p.i64++, (double) 0.0);
                  if (fp)
                    fputs(curScrat, fp);
                  else
                    curScrat += strlen(curScrat);
...
//-------------------------------------------------------------------------
int32_t lux_fprint(ArgumentCount narg, Symbol ps[])
 // (stdout) printing
/* Headers:
   <stdio.h>: FILE, stdout
 */
...
//-------------------------------------------------------------------------
int32_t lux_fprintf(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_close(ArgumentCount narg, Symbol ps[]) /* close subroutine */                
/* Headers:
   <stdio.h>: fclose()
   <stdlib.h>: free()
 */
...
//-------------------------------------------------------------------------
int32_t open_file(ArgumentCount narg, Symbol ps[], char const* access, char function)
 ...
 */
{
 ...
 if (function) {                // function call
   ...
   scalar_value(result_sym).i32 = 0; // default: error
 }
 ...
 if (function)
   scalar_value(result_sym).i32 = 1; // success
 return (function)? result_sym: 1;
}
//-------------------------------------------------------------------------
int32_t lux_openr_f(ArgumentCount narg, Symbol ps[])// openr subroutine
...
//-------------------------------------------------------------------------
int32_t lux_openw_f(ArgumentCount narg, Symbol ps[])// openw subroutine
 // like openr but for (over)writing
{
  return open_file(narg, ps, "w", 1);
}
//-------------------------------------------------------------------------
int32_t lux_openu_f(ArgumentCount narg, Symbol ps[])// openu subroutine
// like openr but for updating
{
  return open_file(narg, ps, "u", 1);
}
//-------------------------------------------------------------------------
int32_t lux_openr(ArgumentCount narg, Symbol ps[])// openr subroutine
/* intended mainly for reading ASCII files in old LUX, but may be useful
   for streams in general for UNIX version */
// associates a file (or a stream) with a lun
{
...
//-------------------------------------------------------------------------
int32_t lux_openw(ArgumentCount narg, Symbol ps[])// openw subroutine
// like openr but for output
{
  return open_file(narg, ps, "w", 0);
}
//-------------------------------------------------------------------------
int32_t lux_openu(ArgumentCount narg, Symbol ps[])// openu subroutine
// like openr but for updating
{
  return open_file(narg, ps, "u", 0);
}
//-------------------------------------------------------------------------
int32_t lux_rewindf(ArgumentCount narg, Symbol ps[]) //rewind file subroutine
/* Headers:
   <stdio.h>: fseek()
 */
{
...
//-------------------------------------------------------------------------
int32_t lux_read(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t readf(ArgumentCount narg, Symbol ps[], int32_t flag)
 // file (stdin) reading and formatting
/* Headers:
   <stdio.h>: FILE
 */
...
//-------------------------------------------------------------------------
int32_t lux_readf(ArgumentCount narg, Symbol ps[])
// file (stdin) reading and formatting
{
  return readf(narg, ps, 1);
}
//-------------------------------------------------------------------------
int32_t lux_readf_f(ArgumentCount narg, Symbol ps[])
// a function version that returns 1 if read OK
{
...
//-------------------------------------------------------------------------
int32_t read_ascii(ArgumentCount narg, Symbol ps[], FILE *fp, int32_t flag)
    ...
    if (!symbolIsNamed(iq)) {        /* an expression of some sort; must be
      ...
      case LUX_SCAL_PTR:
        ...
        case LUX_INT8: case LUX_INT16: case LUX_INT32:
          *scal_ptr_pointer(iq).i32 = value.i32;
          scal_ptr_type(iq) = LUX_INT32;
          break;
        case LUX_INT64:
          *scal_ptr_pointer(iq).i64 = value.i64;
        ...
        if (isNumericalType(array_type(iq))) {
          while (nelem--) {
            ...
            case LUX_INT16:
              switch (type) {
              case LUX_INT8: case LUX_INT16: case LUX_INT32:
                *pp.i16++ = (int16_t) value.i32;
                break;
              case LUX_INT64:
                *pp.i16++ = value.i64;
                break;
              case LUX_FLOAT: case LUX_DOUBLE:
                *pp.i16++ = (int16_t) value.d;
                break;
              }
              break;
            case LUX_INT32:
              switch (type) {
              case LUX_INT8: case LUX_INT16: case LUX_INT32:
                *pp.i32++ = (int32_t) value.i32;
                break;
              case LUX_INT64:
                *pp.i32++ = value.i64;
                break;
              case LUX_FLOAT: case LUX_DOUBLE:
                *pp.i32++ = (int32_t) value.d;
                break;
              }
              break;
            case LUX_INT64:
...
//-------------------------------------------------------------------------
int32_t read_formatted_ascii(ArgumentCount narg, Symbol ps[], void *ptr, int32_t showerrors,
                         int32_t isString)
    ...
    if (theFormat.type != FMT_PLAIN) {
      ...
      if (!(theFormat.flags & FMT_SUPPRESS)) {
        ...
        if (theFormat.count > 1) {                // we need an array
          ...
          trgt.i32 = (int32_t*) array_data(*ps);
        } else {                // only one item
          if (type == LUX_TEMP_STRING) {
            undefine(*ps);
...
//-------------------------------------------------------------------------
int32_t lux_freadf(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_fread(ArgumentCount narg, Symbol ps[])
// FREAD,format,var,...
/* Headers:
   <stdio.h>: stdin
 */
{
  ...
  switch (result) {
    ...
    default:
      ...
      scalar_value(iq).i32 = result;
      break;
  }
  return iq;
}
//-------------------------------------------------------------------------
int32_t lux_freadf_f(ArgumentCount narg, Symbol ps[])
// FREADF(lun,format,var,...)
/* Headers:
   <stdio.h>: FILE
 */
...
//-------------------------------------------------------------------------
int32_t lux_assoc_input(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_fzinspect(ArgumentCount narg, Symbol ps[])                // fzinspect subroutine
...
//-------------------------------------------------------------------------
int32_t fzhead(ArgumentCount narg, Symbol ps[], int32_t flag) // fzhead subroutine
...
//-------------------------------------------------------------------------
int32_t fzread(ArgumentCount narg, Symbol ps[], int32_t flag) // fzread subroutine
 ...
 */
{
  ...
  if ((fh->subf & 1) == 1 ) {        // compression case
    ...
    switch (ch.type) {
      case 0:
        iq = anadecrunch((uint8_t *) p, q1.i16, sbit, ch.bsize, ch.nblocks);
        break;
      case 1:
        iq = anadecrunch8((uint8_t *) p, q1.ui8, sbit, ch.bsize, ch.nblocks);
        break;
      case 2:
        iq = anadecrunchrun((uint8_t *) p, q1.i16, sbit, ch.bsize, ch.nblocks);
        break;
      case 3:
      ...
      case 4:
#if SIZEOF_LONG_LONG_INT == 8        // 64-bit integers
        iq = anadecrunch32((uint8_t *) p, q1.i32, sbit, ch.bsize, ch.nblocks);
#else
...
//-------------------------------------------------------------------------
int32_t fzwrite(ArgumentCount narg, Symbol ps[], int32_t flag) // fzwrite subroutine
 ...
 */
{
  ...
  if (type == LUX_STRING_ARRAY) {
    ...
    j = fwrite(q1.i32, 1, n, fout);        // write data
...
int32_t fcrunwrite(ArgumentCount narg, Symbol ps[], int32_t flag)// fcrunwrite subroutine
...
//-------------------------------------------------------------------------
int32_t fcwrite(ArgumentCount narg, Symbol ps[], int32_t flag)/* fcwrite subroutine */        
 ...
 */
{
 ...
 limit = 2*n;
 q2.i32 = (int32_t *) malloc(limit);        // some room for mistakes
 switch (type) {
   case LUX_INT8:
     if (runlengthflag)
   ...
   case LUX_INT16:
     if (runlengthflag)
       iq = anacrunchrun(q2.ui8, q1.i16, crunch_slice, nx, ny, limit);
     else
       iq = anacrunch(q2.ui8, q1.i16, crunch_slice, nx, ny, limit);
     break;
#if SIZEOF_LONG_LONG_INT == 8
   case LUX_INT32:
     if (!runlengthflag)
       iq = anacrunch32(q2.ui8, q1.i32, crunch_slice, nx, ny, limit);
...
//-------------------------------------------------------------------------
int32_t lux_fzread(ArgumentCount narg, Symbol ps[])
 // routine version
{ return fzread(narg, ps, 0); }
//-------------------------------------------------------------------------
int32_t lux_fzwrite(ArgumentCount narg, Symbol ps[])
 // routine version
{ return fzwrite(narg, ps, 0); }
//-------------------------------------------------------------------------
int32_t lux_fzhead(ArgumentCount narg, Symbol ps[])
 // routine version
{ return fzhead(narg, ps, 0); }
//-------------------------------------------------------------------------
int32_t lux_fcwrite(ArgumentCount narg, Symbol ps[])
 // routine version
{ return fcwrite(narg, ps, 0); }
//-------------------------------------------------------------------------
int32_t lux_fcrunwrite(ArgumentCount narg, Symbol ps[])
{ return fcrunwrite(narg, ps, 0); }
//-------------------------------------------------------------------------
int32_t lux_fzread_f(ArgumentCount narg, Symbol ps[])
 // a function version that returns 1 if read OK
 { return (fzread(narg, ps, 1) == 1)? 1: 4; }
//-------------------------------------------------------------------------
int32_t lux_fzhead_f(ArgumentCount narg, Symbol ps[])
 // a function version that returns 1 if read OK
 { return (fzhead(narg, ps, 1) == 1)? 1: 4; }
//-------------------------------------------------------------------------
int32_t lux_fzwrite_f(ArgumentCount narg, Symbol ps[])
 // function version
{ return (fzwrite(narg, ps, 1) == 1)? 1: 4; }
//-------------------------------------------------------------------------
int32_t lux_fcwrite_f(ArgumentCount narg, Symbol ps[])
 // function version
{ return (fcwrite(narg, ps, 1) == 1)? 1: 4; }
//-------------------------------------------------------------------------
int32_t lux_fcrunwrite_f(ArgumentCount narg, Symbol ps[])
{ return (fcrunwrite(narg, ps, 1) == 1)? LUX_OK: LUX_ERROR; }
//-------------------------------------------------------------------------
int32_t readu(ArgumentCount narg, Symbol ps[], int32_t flag)
...
//-------------------------------------------------------------------------
int32_t writeu(ArgumentCount narg, Symbol ps[], int32_t flag)
...
//-------------------------------------------------------------------------
int32_t lux_readu(ArgumentCount narg, Symbol ps[])
 // subroutine version
{ return readu(narg, ps, 0); }
//-------------------------------------------------------------------------
int32_t lux_readu_f(ArgumentCount narg, Symbol ps[])
 // function version
{ return (readu(narg, ps, 1) == 1)? 1: 4; }
//-------------------------------------------------------------------------
int32_t lux_writeu(ArgumentCount narg, Symbol ps[])
 // subroutine version
{ return writeu(narg, ps, 0); }
//-------------------------------------------------------------------------
int32_t lux_writeu_f(ArgumentCount narg, Symbol ps[])
 // function version
...
void astore_one(FILE *fp, int32_t iq)
{
  ...
  case LUX_CLIST:
    ...
    p.i16 = clist_symbols(iq);
    fwrite(&n, 1, sizeof(n), fp);
    for (i = 0; i < n; i++)
      astore_one(fp, p.i16[i]);
    break;
  }
}
//-------------------------------------------------------------------------
int32_t astore(ArgumentCount narg, Symbol ps[], int32_t flag)
/* STORE,x1 [, x2, x3, ...], file
...
//-------------------------------------------------------------------------
int32_t lux_astore(ArgumentCount narg, Symbol ps[])
{
  return astore(narg, ps, 1);
}
//-------------------------------------------------------------------------
int32_t lux_astore_f(ArgumentCount narg, Symbol ps[])
{
...
int32_t arestore_one(FILE* fp, int32_t iq, int32_t reverseOrder)
{
  ...
  case LUX_CLIST:
    ...
    if (reverseOrder)
      endian(&n, sizeof(int32_t), LUX_INT32);
    ALLOCATE(p.i16, n*sizeof(int16_t), int16_t);
    clist_symbols(iq) = p.i16;
    for (j = 0; j < n; j++) {
...
//-------------------------------------------------------------------------
int32_t arestore(ArgumentCount narg, Symbol ps[], int32_t flag)
...
//-------------------------------------------------------------------------
int32_t lux_arestore(ArgumentCount narg, Symbol ps[])
{
  return arestore(narg, ps, 1);
}
//-------------------------------------------------------------------------
int32_t lux_arestore_f(ArgumentCount narg, Symbol ps[])
     // function form of lux_restore
{
  return arestore(narg, ps, 0);
}
//-------------------------------------------------------------------------
int32_t fileptr(ArgumentCount narg, Symbol ps[], char function)
...
//-------------------------------------------------------------------------
int32_t lux_fileptr(ArgumentCount narg, Symbol ps[])
{
  return fileptr(narg, ps, (char) 0);
}
//-------------------------------------------------------------------------
int32_t lux_fileptr_f(ArgumentCount narg, Symbol ps[])
{
  return fileptr(narg, ps, (char) 1);
}
//-------------------------------------------------------------------------
int32_t lux_dump_lun(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_get_lun(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_chdir(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_filesize(ArgumentCount narg, Symbol ps[])
 ...
 */
{
  ...
  if (fp) {
    fseek(fp, 0, SEEK_END);
    scalar_value(result).i32 = ftell(fp);
    fclose(fp);
  } else
...
//-------------------------------------------------------------------------
int32_t lux_freads(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_freads_f(ArgumentCount narg, Symbol ps[])
     // FREADS(<string>,<format>,<arg1>,...) reads arguments from the
...
//-------------------------------------------------------------------------
int32_t lux_file_to_fz(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_findfile(ArgumentCount narg, Symbol ps[])
...
#define REG_BASIC        0
#endif
int32_t lux_getmatchedfiles(ArgumentCount narg, Symbol ps[])// get all file names that match
 ...
 */
{
 ...
 //-------------------------------------------------------------------------
#if HAVE_REGEX_H
int32_t lux_getmatchedfiles_r(ArgumentCount narg, Symbol ps[])        // also search subdir
...
// <regex.h>
int32_t lux_getfiles_r(ArgumentCount narg, Symbol ps[])        // also search subdir
...
int32_t lux_getfiles_m(ArgumentCount narg, Symbol ps[], int32_t match_flag)
 ...
 */
{
 ...
 //-------------------------------------------------------------------------
int32_t lux_getfiles(ArgumentCount narg, Symbol ps[], int32_t match_flag)
{
...
// uses <regex.h>
int32_t lux_getdirectories(ArgumentCount narg, Symbol ps[], int match_flag)
...
#endif
//-------------------------------------------------------------------------
int32_t lux_identify_file(ArgumentCount narg, Symbol ps[])
...
//-------------------------------------------------------------------------
int32_t lux_hex(ArgumentCount narg, Symbol ps[])
 ...
 */
{
  ...
  for (i=0;i<narg;i++) {
    ...
    switch (symbol_class(iq))        {                
      ...
      case LUX_SCALAR:                //scalar case
        switch (sym[iq].type) {
          ...
          case 1: fprintf(fp, "    %#6x",sym[iq].spec.scalar.i16); break;
          case 2: fprintf(fp, "%#10x",sym[iq].spec.scalar.i32); break;
    ...
    //print entire array, number per line depends on type
            switch (jq) {
              case 2:  p1.i32 = (int32_t *)ptr; k=8; for (j=0;j<nelem;j++)
              { fprintf(fp, "%#10x",*p1.i32++); if (j%8 == 7) fprintf(fp, "\n");}  break;
              ...
              case 1:  p1.i16 = (short *)ptr; k=8; for (j=0;j<nelem;j++)
              { fprintf(fp, "    %#6x",*p1.i16++); if (j%8 == 7) fprintf(fp, "\n");}  break;
    ...
    switch (type) {
      case LUX_INT8:
        switch (type0) {
          ...
          case LUX_INT16:
            while (nelem--) {
              *q.ui8 = (*p.i16 == blank)? targetblank: *p.i16 * bscale + bzero;
              q.ui8--;
              p.i16--;
            }
            break;
          case LUX_INT32:
            while (nelem--) {
              *q.ui8 = (*p.i32 == blank)? targetblank: *p.i32 * bscale + bzero;
              q.ui8--;
              p.i32--;
            }
            break;
          case LUX_INT64:
            while (nelem--) {
              *q.ui8 = (*p.i64 == blank)? targetblank: *p.i64 * bscale + bzero;
              q.ui8--;
              p.i64--;
            }
            break;
      ...
      case LUX_INT16:
        switch (type0) {
          case LUX_INT8:
            while (nelem--) {
              *q.i16 = (*p.ui8 == blank)? targetblank: *p.ui8 * bscale + bzero;
              q.i16--;
              p.ui8--;
            }
            break;
          case LUX_INT16:
            while (nelem--) {
              *q.i16 = (*p.i16 == blank)? targetblank: *p.i16 * bscale + bzero;
              q.i16--;
              p.i16--;
            }
            break;
          case LUX_INT32:
            while (nelem--) {
              *q.i16 = (*p.i32 == blank)? targetblank: *p.i32 * bscale + bzero;
              q.i16--;
              p.i32--;
            }
            break;
          case LUX_INT64:
            while (nelem--) {
              *q.i16 = (*p.i64 == blank)? targetblank: *p.i64 * bscale + bzero;
              q.i16--;
              p.i64--;
            }
            break;
      ...
      case LUX_INT32:
        switch (type0) {
          case LUX_INT8:
            while (nelem--) {
              *q.i32 = (*p.ui8 == blank)? targetblank: *p.ui8 * bscale + bzero;
              q.i32--;
              p.ui8--;
            }
            break;
          case LUX_INT16:
            while (nelem--) {
              *q.i32 = (*p.i16 == blank)? targetblank: *p.i16 * bscale + bzero;
              q.i32--;
              p.i16--;
            }
            break;
          case LUX_INT32:
            while (nelem--) {
              *q.i32 = (*p.i32 == blank)? targetblank: *p.i32 * bscale + bzero;
              q.i32--;
              p.i32--;
            }
            break;
          case LUX_INT64:
            while (nelem--) {
              *q.i32 = (*p.i64 == blank)? targetblank: *p.i64 * bscale + bzero;
              q.i32--;
              p.i64--;
            }
            break;
      ...
      case LUX_FLOAT:
        switch (type0) {
          ...
          case LUX_INT16:
            while (nelem--) {
              *q.f = (*p.i16 == blank)? targetblank: *p.i16 * bscale + bzero;
              q.f--;
              p.i16--;
            }
            break;
          case LUX_INT32:
            while (nelem--) {
              *q.f = (*p.i32 == blank)? targetblank: *p.i32 * bscale + bzero;
              q.f--;
              p.i32--;
            }
            break;
          case LUX_INT64:
            while (nelem--) {
              *q.f = (*p.i64 == blank)? targetblank: *p.i64 * bscale + bzero;
              q.f--;
              p.i64--;
            }
            break;
...
int32_t lux_fits_read_general(ArgumentCount narg, Symbol ps[], int32_t func)// read fits files
...
//-------------------------------------------------------------------------
int32_t lux_fits_read(ArgumentCount narg, Symbol ps[])
{
  return lux_fits_read_general(narg, ps, 0);
}
//-------------------------------------------------------------------------
int32_t lux_fits_read_f(ArgumentCount narg, Symbol ps[])
{
  return lux_fits_read_general(narg, ps, 1);
}
//-------------------------------------------------------------------------
int32_t lux_fits_header_f(ArgumentCount narg, Symbol ps[])// read fits header
...
//-------------------------------------------------------------------------
int32_t lux_fits_xread_f(ArgumentCount narg, Symbol ps[])// read fits extension and headers
...
//-------------------------------------------------------------------------
int32_t lux_fits_write_general(ArgumentCount narg, Symbol ps[], int32_t func)
...
//-------------------------------------------------------------------------
int32_t lux_fits_write(ArgumentCount narg, Symbol ps[])
{
  return lux_fits_write_general(narg, ps, 0);
}
//-------------------------------------------------------------------------
int32_t lux_fits_write_f(ArgumentCount narg, Symbol ps[])
{
  return lux_fits_write_general(narg, ps, 1);
}
//-------------------------------------------------------------------------
int32_t lux_fileread(ArgumentCount narg, Symbol ps[])
 ...
 */
{
 ...
 //-------------------------------------------------------------------------
int32_t lux_filewrite(ArgumentCount narg, Symbol ps[])
 ...
 */
{
  ...
  switch (symbol_class(iq)) {
    ...
    case LUX_SCALAR:
      p = (char *) &scalar_value(iq).i32;        //scalar case
