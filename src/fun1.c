  ...
  if (type < LUX_DOUBLE) {
   ...
   /* need a first pass to get the mean value */
	...
	case LUX_BYTE:
      ...
      sdev = sqrt(sdev/(nc-1));
      redef_scalar(ps[4], LUX_FLOAT, &sdev);
      if (skew_flag) {
	skew = skew/(nc*pow(sdev, 3));
	redef_scalar(ps[5], LUX_FLOAT, &skew);
      }
      if (kurtosis_flag) {
	kurtosis = kurtosis/(nc*pow(sdev, 4) - 3.0);
	redef_scalar(ps[6], LUX_FLOAT, &kurtosis);
      }
    }
    return LUX_OK;
  } else {
    ...
    mean = s * fac;
    /* finish the mean returned value now */
    redef_scalar(ps[3], LUX_DOUBLE, &mean);
    redef_scalar(ps[1], LUX_DOUBLE, &max);
    redef_scalar(ps[2], LUX_DOUBLE, &min);
    /* need a second pass unless only mean was required */
    if (sdev_flag) {
      n = nc;
      q1.l = save_ptr;
	...
	if (skew_flag) { 
      ...
      sdev = sqrt(sdev/(nc-1));
      redef_scalar(ps[4], LUX_DOUBLE, &sdev);
      if (skew_flag) {
	skew = skew/(nc*pow(sdev, 3));
	redef_scalar(ps[5], LUX_DOUBLE, &skew);
      }
      if (kurtosis_flag) {
	kurtosis = kurtosis/(nc*pow(sdev, 4) - 3.0);
	redef_scalar(ps[6], LUX_DOUBLE, &kurtosis);
      }
    }
    return LUX_OK;
  }
