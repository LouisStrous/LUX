...
*/
{
 ...
 if (array_type(ps[1]) > type) {
   ...
   p1.l = (int32_t*) array_data(lux_converts[type](1, &ps[0]));
   p2.l = (int32_t*) array_data(ps[1]);
 } else {
   p1.l = (int32_t*) array_data(ps[0]);
   p2.l = (int32_t*) array_data(lux_converts[type](1, &ps[1]));
 }
 dstype = type;
