...
BIND(iauBi00, v_dpT3_r3_000, f, bi00, 0, 0, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* bp00, jd, rb, rp, rbp
...
BIND(iauBp00, v_dddp33T3_iao33op3p3qocq_0z1T3, s, bp00, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* bp06, jd, rb, rp, rbp
 ...
 */
BIND(iauC2ixy, v_dT4dp33_iaiqiqrp3p3q_0z1T3, f, c2ixy, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* rc2i = c2ixys(x, y, s)
 ...
 */
BIND(iauC2ixys, v_dT3dp33_iaiqiqrp3p3q_0T3, f, c2ixys, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* angles = c2s(p)
   ...
   */
BIND(iauC2s, v_dp3dpdp_i3arm3p2q_011, f, c2s, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauC2t00a, v_dT6dp33_iaiqT3rp3p3q_0z11T4, f, c2t00a, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* rc2t = c2t00b(jdtt, jdut, x, y)
   ...
   model.  <jdtt> and <jdut> should express the same instants in time,
   in the TT and UT1 timescales, respectively */
BIND(iauC2t00b, v_dT6dp33_iaiqT3rp3p3q_0z11T4, f, c2t00b, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* rc2t = c2t06a(jd, x, y)
   ...
   respectively */
BIND(iauC2t06a, v_dT6dp33_iaiqT3rp3p3q_0z11T4, f, c2t06a, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauC2tcio, v_dp33ddp33T2_i33aimmqiqrq_0T3, f, c2tcio, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauC2teqx, v_dp33ddp33T2_i33aimmqiqrq_0T3, f, c2teqx, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* c2tpe(jd, dpsi, deps, xp, yp)
   ...
   <yp> */
BIND(iauC2tpe, v_dT8dp33_iaiqT4rp3p3q_0zzz1T5, f, c2tpe, 5, 5, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* c2txy(jd, x, y, xp, yp)
   ...
   <yp> */
BIND(iauC2txy, v_dT8dp33_iaiqT4rp3p3q_0zzz1T5, f, c2txy, 5, 5, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* cal2jd(<caldates>)
...
int32_t lux_iauCal2jd(ArgumentCount narg, Symbol ps[])
{
  double djm0, djm;
  Pointer *ptrs;
  LoopInfo *infos;
  ...
  case LUX_INT32:
    while (infos[1].nelem--) {
      if (iauCal2jd(ptrs[0].i32[0], ptrs[0].i32[1], ptrs[0].i32[2], &djm0, &djm))
        *ptrs[1].i32 = 0;
      else
        *ptrs[1].i32 = (djm0 + 0.5) + djm;
      ptrs[0].i32 += 3;
      ptrs[1].i32++;
    }
    break;
...
REGISTER(iauCal2jd, f, cal2jd, 1, 1, 0, HAVE_LIBSOFA_C);
//-----------------------------------------------------------------------
/* dat(date) returns delta(AT) = TAI - UTC for the given UTC date
   ([year, month, day]) */
int32_t lux_iauDat(ArgumentCount narg, Symbol ps[])
{
  Pointer *ptrs;
  LoopInfo *infos;
  int32_t iq;
  ...
  if (internalMode & 1) {       // /VALID
    ...
    case LUX_INT32:
      while (infos[1].nelem--) {
        iauDat(ptrs[0].i32[0], ptrs[0].i32[1], ptrs[0].i32[2], 0.0, ptrs[1].d++);
        ptrs[0].i32 += 3;
      }
      break;
    case LUX_INT64:
...
REGISTER(iauDat, f, dat, 0, 1, "1valid", HAVE_LIBSOFA_C);
//-----------------------------------------------------------------------
...
BIND(iauDtdb, d_dT6_iaiT4rq_0z1T4_5, f, dtdb, 5, 5, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* ee00(<jd>, <epsa>, <dpsi>)
...
BIND(do_iauEe00, d_dT3_iaiqiqrq_012, f, ee00, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* ee00a(<jd>)

   Returns the equation of the equinoxes, compatible with IAU 2000
   resolutions */
BIND(iauEe00a, d_dd_iarq_0z_1, f, ee00a, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* ee00b(<jd>)

   Returns the equation of the equinoxes, compatible with IAU 2000
   resolutions but using the truncated nutation model IAU 2000B */
BIND(iauEe00b, d_dd_iarq_0z_1, f, ee00b, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* ee06a(<jd>)

   Returns the equation of the equinoxes, compatible with IAU 2000
   resolutions and the IAU 2006/200A precession-nutation */
BIND(iauEe06a, d_dd_iarq_0z_1, f, ee06a, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* eect00(<jd>)

   Returns the equation of the equinoxes complementary term,
   consistent with IAU 2000 resolutions. */
BIND(iauEect00, d_dd_iarq_0z_1, f, eect00, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* eform(<ellid>)
   ...
   returned. */
BIND(iauEform, i_idpdp_iarp2q_011, f, eform, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* eo06a(<jd>)

   Returns the equation of the origins, using IAU 2006 precession and
   IAU 2000A nutation. */
BIND(iauEo06a, d_dd_iarq_0z_1, f, eo06a, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* eors(<rnpb>, <s>)

   Returns the equation of the origins, given the classical NPB matrix
   <rnpb> and the CIO locator <s> */
BIND(iauEors, d_dp33d_i33aimmqrcq_01_2, f, eors, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* epb(<jd>)

   Returns the Besselian epoch corresponding to Julian Date <jd>. */
BIND(iauEpb, d_dd_iarq_0z_1, f, epb, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* epb2jd(<bepoch>)

   Returns the Julian Date corresponding to Besselian epoch <bepoch>
   (e.g., 1957.3). */
int32_t lux_iauEpb2jd(ArgumentCount narg, Symbol ps[])
{
  Pointer *ptrs;
  LoopInfo *infos;
...
REGISTER(iauEpb2jd, f, epb2jd, 1, 1, 0, HAVE_LIBSOFA_C);
//-----------------------------------------------------------------------
/* epj(<jd>)

   Returns the Julian epoch corresponding to Julian Date <jd>. */
BIND(iauEpj, d_dd_iarq_0z_1, f, epj, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* epj2jd(<bepoch>)

   Returns the Julian Date corresponding to Julian epoch <bepoch>
   (e.g., 1957.3). */
int32_t lux_iauEpj2jd(ArgumentCount narg, Symbol ps[])
{
  Pointer *ptrs;
  LoopInfo *infos;
...
REGISTER(iauEpj2jd, f, epj2jd, 1, 1, 0, HAVE_LIBSOFA_C);
//-----------------------------------------------------------------------
/* epv00, jd, pvh, pvb
...
BIND(iauEpv00, i_dddp23T2_iaop2p3qocq_0z12, s, epv00, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* eqeq94(<jd>)

   Returns the equation of the equinoxes according to the IAU 1994 model.
 */
BIND(iauEqeq94, d_dd_iarq_0z_1, f, eqeq94, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* era00(<jd>)

   Returns the Earth rotation angle according to the IAU 2000 model.
*/
BIND(iauEra00, d_dd_iarq_0z_1, f, era00, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fad03(<t>)
   ...
   instead. */
BIND(iauFad03, d_d_iarq_0_1, f, fad03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fae03(<t>)
   ...
   instead. */
BIND(iauFae03, d_d_iarq_0_1, f, fae03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* faf03(<t>)
...
BIND(iauFaf03, d_d_iarq_0_1, f, faf03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* faju03(<t>)
   ...
   practical difference if <t> is measured since J2000.0 TT
   instead. */
BIND(iauFaju03, d_d_iarq_0_1, f, faju03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fal03(<t>)
   ...
   practical difference if <t> is measured since J2000.0 TT
   instead. */
BIND(iauFal03, d_d_iarq_0_1, f, fal03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* falp03(<t>)
   ...
   practical difference if <t> is measured since J2000.0 TT
   instead. */
BIND(iauFalp03, d_d_iarq_0_1, f, falp03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fama03(<t>)
   ...
   practical difference if <t> is measured since J2000.0 TT
   instead. */
BIND(iauFama03, d_d_iarq_0_1, f, fama03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fame03(<t>)
   ...
   practical difference if <t> is measured since J2000.0 TT
   instead. */
BIND(iauFame03, d_d_iarq_0_1, f, fame03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fane03(<t>)
   ...
   practical difference if <t> is measured since J2000.0 TT
   instead. */
BIND(iauFane03, d_d_iarq_0_1, f, fane03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* faom03(<t>)
   ...
   TT instead. */
BIND(iauFaom03, d_d_iarq_0_1, f, faom03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fapa03(<t>)
   ...
   It makes no practical difference if <t> is measured since J2000.0
   TT instead. */
BIND(iauFapa03, d_d_iarq_0_1, f, fapa03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fasa03(<t>)
   ...
   practical difference if <t> is measured since J2000.0 TT
   instead. */
BIND(iauFasa03, d_d_iarq_0_1, f, fasa03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* faur03(<t>)
   ...
   practical difference if <t> is measured since J2000.0 TT
   instead. */
BIND(iauFaur03, d_d_iarq_0_1, f, faur03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fave03(<t>)
   ...
   practical difference if <t> is measured since J2000.0 TT
   instead. */
BIND(iauFave03, d_d_iarq_0_1, f, fave03, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauFk52h, v_dT6dpT6_iaiqT5oqT6_0T11, s, fk52h, 12, 12, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fk5hip, <r5h>, <s5h>

   Returns the FK5-to-Hipparcos rotation <r5h> and spin <s5h> */
BIND(iauFk5hip, v_dp3dp_o33o3_01, s, fk5hip, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauFk5hz, v_dT4dpdp_iaiqiqoqoq_0T2z34, s, fk5hz, 5, 5, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fw2m(<gamma_bar>, <phi_bar>, <psi>, <eps>)

   Forms a rotation matrix given the Fukushima-Williams angles
   <gamma_bar>, <phi_bar>, <psi>, and <eps> */
BIND(iauFw2m, v_dT4dp33_iaiqT3rp3p3q_0T4, f, fw2m, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* fw2xy, <gamma_bar>, <phi_bar>, <psi>, <eps>, <x>, <y>
   ...
   <eps>. */
BIND(iauFw2xy, v_dT4dpdp_iaiqT3oqoq_0T5, s, fw2xy, 6, 6, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gc2gd(<xyz>, <ellid>)
...
BIND(iauGc2gd, i_idp3dpT3_ii3arcq_0T222, f, gc2gd, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gc2gde(<xyz>, <a>, <f>)
...
BIND(iauGc2gde, v_dddp3dpT3_i3aiirq_120333, f, gc2gde, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gd2gc(<eph>, <ellid>)
   ...
   reference ellipsoid <ellid> */
BIND(iauGd2gc, i_idT3dp3_i3airq_10002, f, gd2gc, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gd2gce(<eph>, <a>, <f>)
   ...
   ellipsoid with equatorial radius <a> and flattening <f> */
BIND(iauGd2gce, i_dT5dp3_i3aiirq_120003, f, gd2gce, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gmst00(<jdut>, <jdtt>)
...
BIND(iauGmst00, d_dT4_iaiqrq_01_2, f, gmst00, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gmst06(<jdut>, <jdtt>)
   ...
   effects of precession.  If <jdtt> is set equal to <jdut> then
   errors of order 100 microarcseconds result. */
BIND(iauGmst06, d_dT4_iaiqrq_01_2, f, gmst06, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gmst82(<jd>)

   Returns Greenwich mean sidereal time for the given UT1 Julian Date
   <jd>, according to the IAU 1982 model */
BIND(iauGmst82, d_dd_iarq_0z_1, f, gmst82, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gst00a(<jdut>, <jdtt>)
...
BIND(iauGst00a, d_dT4_iaiqrq_01_2, f, gst00a, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gst00b(<jdut>)
...
BIND(iauGst00b, d_dd_iarq_0z_1, f, gst00b, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gst06(<jdut>, <jdtt>, <rnpb>)
   ...
   effects of precession.  If <jdtt> is set equal to <jdut> then
   errors of order 100 microarcseconds result. */
BIND(iauGst06, d_dT4dp33_iaiqip3p3qrq_00112_3, f, gst06, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gst06a(<jdut>, <jdtt>)
...
BIND(iauGst06a, d_dT4_iaiqrq_01_2, f, gst06a, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* gst94(<jd>)

   Returns Greenwich apparent sidereal time for the given UT1 Julian
   Date <jd>, consistent with IAU 1982/94 resolutions */
BIND(iauGst94, d_dd_iarq_0z_1, f, gst94, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauH2fk5, v_dT6dpT6_iaiqT5oqT6_0T11, s, h2fk5, 12, 12, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* hfk5z, <rah>, <dech>, <jd>, <ra5>, <dec5>
   ...
   declination <dec5> */
BIND(iauHfk5z, v_dT4dpT4_iaiqiqoqT4_0T2z3T6, s, hfk5z, 7, 7, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* ir()

   Returns an identity r-matrix */
BIND(iauIr, v_dp33_r33_0, f, ir, 0, 0, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauJd2cal, i_ddipT3dp_iarp3q_0z1111, f, jd2cal, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* num00a(<jd>)

   Returns the matrix for nutation for a given Julian Date <jd>,
   according to the IAU 2000A model. */
BIND(iauNum00a, v_dddp33_iarp3p3q_0z1, f, num00a, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* num00b(<jd>)

   Returns the matrix for nutation for a given Julian Date <jd>,
   according to the IAU 2000B model. */
BIND(iauNum00b, v_dddp33_iarp3p3q_0z1, f, num00b, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* num06a(<jd>)

   Returns the matrix for nutation for a given Julian Date <jd>,
   according to the IAU 2006/2000A model. */
BIND(iauNum06a, v_dddp33_iarp3p3q_0z1, f, num06a, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauNumat, v_dT3dp33_iaiqiqrp3p3q_0T3, f, numat, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* nut00a, <jd>, <dpsi>, <deps>
...
BIND(iauNut00a, v_dddpdp_iaoqoq_0z12, s, nut00a, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* nut00b, <jd>, <dpsi>, <deps>
...
BIND(iauNut00b, v_dddpdp_iaoqoq_0z12, s, nut00b, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* nut06a, <jd>, <dpsi>, <deps>
   ...
   + planetary) */
BIND(iauNut06a, v_dddpdp_iaoqoq_0z12, s, nut06a, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* nut80, <jd>, <dpsi>, <deps>
...
BIND(iauNut80, v_dddpdp_iaoqoq_0z12, s, nut80, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* nutm80(<jd>)
   ...
   IAU 1980 model.
 */
BIND(iauNutm80, v_dddp33_iarp3p3q_0z1, f, nutm80, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* obl06(<jd>)
 ...
 */
BIND(iauObl06, d_dd_iarq_0z_1, f, obl06, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* obl80(<jd>)

   Returns the mean obliquity of the ecliptic according to the IAU
   1980 model. */
BIND(iauObl80, d_dd_iarq_0z_1, f, obl80, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* p06e, <jd>, <eps0>, <psia>, <oma>, <bpa>, <bqa>, <pia>, <bpia>,
   ...
   F-W angle psi_J2000.
 */
BIND(iauP06e, v_dddpT16_iaoqT16_0z1T16, s, p06e, 17, 17, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* p2s(<p>)

   Returns the spherical polar coordinates (theta, phi, r)
   corresponding to the p-vector <p> */
BIND(iauP2s, v_dp3dpT3_i3arq_0111, f, p2s, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pap(<ref>, <tgt>)

   Returns the position angle of p-vector <tgt> from p-vector <ref>
*/
BIND(iauPap, d_dp3dp3_i3aiqrm3q_01_2, f, pap, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pas(<al>, <ap>, <bl>, <bp>)
   ...
   point B in radians
*/
BIND(iauPas, d_dT4_iaiqT3rq_0T3_4, f, pas, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauPb06, v_dddpT3_iaoqT3_0z1T3, s, pb06, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pdp(<a>, <b>)

   Returns the inner product of the two p-vectors <a> and <b> */
BIND(iauPdp, d_dp3dp3_i3aiqrm3q_01_2, f, pdp, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pfw06, <jd>, <gamb>, <phib>, <psib>, <epsa>

   Returns the IAU 2006 precession angles in Fukushima-Williams
   4-angle formulation. */
BIND(iauPfw06, v_dddpT4_iaoqT4_0z1T4, s, pfw06, 5, 5, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* plan94(<jd>, <planet>)
...
*/
BIND(iauPlan94, i_ddidp23_iairp3p2q_0z12, f, plan94, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pm(<p>)

   Returns the modulus of the p-vector <p>
*/
BIND(iauPm, d_dp3_i3arm3q_0_1, f, pm, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pmat00(<jd>)
...
*/
BIND(iauPmat00, v_dddp33_iarp3p3q_0z1, f, pmat00, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pmat06(<jd>)
...
*/
BIND(iauPmat06, v_dddp33_iarp3p3q_0z1, f, pmat06, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pmat76(<jd>)

   Returns the precession matrix from J2000.0 to a specified Julian
   Date <jd> according to the IAU 1976 model. */
BIND(iauPmat76, v_dddp33_iarp3p3q_0z1, f, pmat76, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pn, <p>, <r>, <u>

   Converts p-vector <p> into a modulus <r> and unit vector <u> */
BIND(iauPn, v_dp3dpdp3_i3aomqoq_0T2, s, pn, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauPn00, v_dT4dpdp3T5_iaiqiqoqop3p3qocqT4_0z1T8, s, pn00, 9, 9, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pn00a, <jd>, <dpsi>, <deps>, <epsa>, <rb>, <rp>, <rbp>, <rn>, <rbpn>
   ...
   <rb>, the precession matrix <rp>, the bias-precession matrix <rbp>,
   the nutation matrix <rn>, and the GCRS-to-true matrix <rbpn>. */
BIND(iauPn00a, v_dddpT3dp33T5_iaoqT3op3p3qocqT4_0z1T8, s, pn00a, 9, 9, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pn00b, <jd>, <dpsi>, <deps>, <epsa>, <rb>, <rp>, <rbp>, <rn>, <rbpn>
   ...
   <rb>, the precession matrix <rp>, the bias-precession matrix <rbp>,
   the nutation matrix <rn>, and the GCRS-to-true matrix <rbpn>. */
BIND(iauPn00b, v_dddpT3dp33T5_iaoqT3op3p3qocqT4_0z1T8, s, pn00b, 9, 9, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pn06, <jd>, <dpsi>, <deps>, <epsa>, <rb>, <rp>, <rbp>, <rn>, <rbpn>
   ...
   <rb>, the precession matrix <rp>, the bias-precession matrix <rbp>,
   the nutation matrix <rn>, and the GCRS-to-true matrix <rbpn>. */
BIND(iauPn06, v_dT4dpdp3T5_iaiqiqoqop3p3qocqT4_0z1T8, s, pn06, 9, 9, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pn06a, <jd>, <dpsi>, <deps>, <epsa>, <rb>, <rp>, <rbp>, <rn>, <rbpn>
   ...
   <rbpn>. */
BIND(iauPn06a, v_dddpT3dp33T5_iaoqT3op3p3qocqT4_0z1T8, s, pn06a, 9, 9, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pnm00a(<jd>)

   Returns the matrix of precession-nutation (including frame bias)
   for Julian Date <jd>, equinox-based, IAU 2000A model. */
BIND(iauPnm00a, v_dddp33_iarp3p3q_0z1, f, pnm00a, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pnm00b(<jd>)

   Returns the matrix of precession-nutation (including frame bias)
   for Julian Date <jd>, equinox-based, IAU 2000B model. */
BIND(iauPnm00b, v_dddp33_iarp3p3q_0z1, f, pnm00b, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pnm06a(<jd>)
...
BIND(iauPnm06a, v_dddp33_iarp3p3q_0z1, f, pnm06a, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pnm80(<jd>)
...
/* prec76, <jd1>, <jd2>, <zeta>, <z>, <theta>
   ...
   the y axis. */
BIND(iauPrec76, v_dT4dpT3_iaiqoqT3_0z1z2T4, s, prec76, 5, 5, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
...
BIND(iauPv2s, v_dp23dpT6_iD23aommqocqT5_0T6, s, pv25, 7, 7, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pvdpv(<a>, <b>)

   Returns the inner product of pv-vectors <a> and <b> */
BIND(iauPvdpv, v_dp23T2dp2_i23aiqrkmq_0T2, f, pvdpv, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pvm, <pv>, <r>, <s>
   ...
   component. */
BIND(iauPvm, v_dp23dpdp_i23aommqocq_0T2, s, pvm, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pvstar, <pv>, <ra>, <dec>, <pmr>, <pmd>, <px>, <rv>
   ...
   receding) */
BIND(iauPvstar, i_dp23dpT6_i23aom2m3qomqT5_0T6, s, pvstar, 7, 7, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pvxpv(<a>, <b>)

   Returns the outer product of two pv-vectors <a> and <b> */
BIND(iauPvxpv, v_dp23T3_i23aiqrq_0T2, f, pvxpv, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* pxp(<a>, <b>)

   Returns the outer product of two p-vectors <a> and <b> */
BIND(iauPxp, v_dp3T3_i3aiqrq_0T2, f, pxp, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* rm2v(<r>)
...
BIND(iauRm2v, v_dp33dp3_i33armq_01, f, rm2v, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* rxp(<r>, <p>)

   Returns the matrix product of an r-matrix and a p-vector. */
BIND(iauRxp, v_dp33dp3T2_i33aimqrcq_0T2, f, rxp, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* rxr(<r1>, <r2>)

   Returns the matrix product of two r-matrices. */
BIND(iauRxr, v_dp33T3_i33aiqrq_0T2, f, rxr, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* rv2m(<w>)
   ...
   <w>. */
BIND(iauRv2m, v_dp3dp33_i3arp3q_01, f, rv2m, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* rx, <phi>, <r>

   Rotates r-matrix <r> about the x axis over angle <phi> */
BIND(iauRx, v_ddp33_iao33q_01, s, rx, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* rxpv(<r>, <pv>)

   Returns the matrix product of r-matrix <r> and pv-vector <pv> */
BIND(iauRxpv, v_dp33dp23T2_i33aim3p2qrcq_0T2, f, rxpv, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* ry, <phi>, <r>

   Rotates r-matrix <r> about the y axis over angle <phi> */
BIND(iauRy, v_ddp33_iao33q_01, s, ry, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* rz, <phi>, <r>

   Rotates r-matrix <r> about the z axis over angle <phi> */
BIND(iauRz, v_ddp33_iao33q_01, s, rz, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* s00(<jd>, <x>, <y>)
...
BIND(do_iauS00, d_dT3_iaiqiqrq_012, f, s00, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* s00a(<jd>)

   Returns the CIO locator for the specified Julian Date <jd>, using
   IAU 2000A precession-nutation */
BIND(iauS00a, d_dd_iarq_0z_1, f, s00a, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* s00b(<jd>)

   Returns the CIO locator for the specified Julian Date <jd>, using
   IAU 2000B precession-nutation */
BIND(iauS00b, d_dd_iarq_0z_1, f, s00b, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* s06(<jd>, <x>, <y>)
...
BIND(do_iauS06, d_dT3_iaiqiqrq_012, f, s06, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* s06a(<jd>)

   Returns the CIO locator for the specified Julian Date <jd>, using
   IAU 2006 precession and IAU 2000A nutation */
BIND(iauS06a, d_dd_iarq_0z_1, f, s06a, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* s2c(<theta>, <phi>)
...
BIND(iauS2c, v_dddp3_iaiqrp3q_0T2, f, S2C, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* s2p(<theta>, <phi>, <r>)

   Converts spherical polar coordinates longitude <theta>, latitude
   <phi>, radial distance <r> to a p-vector */
BIND(iauS2p, v_dT3dp3_iaiqiqrp3q_0T3, f, s2p, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* s2pv(<theta>, <phi>, <r>, <td>, <pd>, <rd>)
   ...
   r <rd> */
BIND(iauS2pv, v_dT6dp23_iaiqT5op2p3q_0T6, f, s2pv, 6, 6, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* sepp(<a>, <b>)

   Returns the angular separation between two p-vectors <a> and <b> */
BIND(iauSepp, d_dp3dp3_i3aiqrm3q_01_2, f, sepp, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* seps(<lon1>, <lat1>, <lon2>, <lat2>)
   ...
   latitude <lat2> */
BIND(iauSeps, d_dT4_iaiqT3rq_0T3_4, f, seps, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* sp00(<jd>)
   ...
   Intermediate Pole */
BIND(iauSp00, d_dd_iarq_0z_1, f, sp00, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* starpm, <ra1>, <dec1>, <pmr1>, <pmd1>, <px1>, <rv1>, <jd1>,
   ...
   *1 = before, *2 = after */
BIND(iauStarpm, i_dT10dpT6_iaiqT7oqT6_0T6z7z8T13, s, starpm, 14, 14, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* starpv(<ra>, <dec>, <pmr>, <pmd>, <px>, <rv>)

   Converts star catalog coordinates to a position-velocity matrix */
BIND(iauStarpv, i_dT6dp23_iaiqT5op2p3q_0T6, f, starpv, 6, 6, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* taitt(<jdtai>)

   Translates Julian Date <jdtai> from TAI to TT */
BIND(iauTaitt, i_dddpdp_iarq_0z11, f, taitt, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* taiut1(<jdtai>, <dta>)

   Translates Julian Date <jdtai> from TAI to UT1, given <dta> = UT1 -
   TAi in seconds */
BIND(iauTaiut1, i_dT3dpdp_iaiqrq_0z122, f, taiut1, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* taiutc(<jdtai>)

   Translates Julian Date <jdtai> from TAI to UTC */
BIND(iauTaiutc, i_dddpdp_iarq_0z11, f, taiutc, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* tcbtdb(<jdtcb>)

   Translates Julian Date <jdtcb> from TCB to TDB */
BIND(iauTcbtdb, i_dddpdp_iarq_0z11, f, tcbtdb, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* tcgtt(<jdtcg>)

   Translates Julian Date <jdtcg> from TCG to TT */
BIND(iauTcgtt, i_dddpdp_iarq_0z11, f, tcgtt, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* tdbtcb(<jdtdb>)

   Translates Julian Date <jdtdb> from TDB to TCB */
BIND(iauTdbtcb, i_dddpdp_iarq_0z11, f, tdbtcb, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* tdbtt(<jdtdb>, <dtr>)

   Translates Julian Date <jdtbd> from TDB to TT, given <dtr> = TDB -
   TT in seconds  */
BIND(iauTdbtt, i_dT3dpdp_iaiqrq_0z122, f, tdbtt, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* tr(<r>)

   Transposes r-matrix <r> */
BIND(iauTr, v_dp33T2_i33arq_01, f, tr, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* trxp(<r>, <p>)

   Multiply p-vector <p> by the transpose of r-matrix <r> */
BIND(iauTrxp, v_dp33dp3T2_i33aimqrcq_0T2, f, trxp, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* trxpv(<r>, <pv>)

   Multiply pv-vector <pv> by the transpose of r-matrix <r> */
BIND(iauTrxpv, v_dp33dp23T2_i33aim3p2qrcq_0T2, f, trxpv, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* tttai(<jdtt>)

   Transforms Julian Date <jdtt> from TT to TAI */
BIND(iauTttai, i_dddpdp_iarq_0z11, f, tttai, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* tttcg(<jdtt>)

   Transforms Julian Date <jdtt> from TT to TCG */
BIND(iauTttcg, i_dddpdp_iarq_0z11, f, tttcg, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* tttdb(<jdtt>, <dt>)

   Transforms Julian Date <jdtt> from TT to TDB, given <dt> = TDB -
   TT in seconds */
BIND(iauTttdb, i_dT3dpdp_iaiqrq_0z122, f, tttdb, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* ttut1(<jdtt>, <dt>)

   Transforms Julian Date <jdtt> from TT to UT1, given <dt> = TT - UT1
   in seconds */
BIND(iauTtut1, i_dT3dpdp_iaiqrq_0z122, f, ttut1, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* ut1tai(<jdut>, <dt>)

   Transforms Julian Date <jdut> from UT1 to TAI, given <dt> = UT1 -
   TAI in seconds */
BIND(iauUt1tai, i_dT3dpdp_iaiqrq_0z122, f, ut1tai, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* ut1tt(<jdut>, <dt>)

   Transforms Julian Date <jdut> from UT1 to TT, given <dt> = UT1 -
   TT in seconds */
BIND(iauUt1tt, i_dT3dpdp_iaiqrq_0z122, f, ut1tt, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* ut1utc(<jdut>, <dt>)

   Transforms Julian Date <jdut> from UT1 to UTC, given <dt> = UT1 -
   UTC in seconds */
BIND(iauUt1utc, i_dT3dpdp_iaiqrq_0z122, f, ut1utc, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* utctai(<jdut>)

   Transforms Julian Date <jdut> from UTC to TAI */
BIND(iauUtctai, i_dddpdp_iarq_0z11, f, utctai, 1, 1, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* utcut1(<jdut>, <dt>)

   Transforms Julian Date <jdut> from UTC to UT1, given <dt> = UT1 -
   UTC in seconds */
BIND(iauUtcut1, i_dT3dpdp_iaiqrq_0z122, f, utcut1, 2, 2, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* xy06, <jd>, <x>, <y>
   ...
   2000A nutation. */
BIND(iauXy06, v_dddpdp_iaoqoq_0z12, s, xy06, 3, 3, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* xys00a, <jd>, <x>, <y>, <s>
...
BIND(iauXys00a, v_dddpT3_iaoqT3_0z1T3, s, xys00a, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* xys00b, <jd>, <x>, <y>, <s>
   ...
   precession-nutation model */
BIND(iauXys00b, v_dddpT3_iaoqT3_0z1T3, s, xys00b, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
/* xys06a, <jd>, <x>, <y>, <s>
...
BIND(iauXys06a, v_dddpT3_iaoqT3_0z1T3, s, xys06a, 4, 4, 0, HAVE_LIBSOFA_C)
//-----------------------------------------------------------------------
#endif
