...
GC        menugc, menurgc;
Cursor        cursor;
int32_t        last_menu, menu_item, fontheight, fontwidth;
float        menu_x, menu_y;
int32_t        text_menus = 0;

void        delete_menu(int32_t);
//------------------------------------------------------------------------
int32_t menu_setup(void)
...
//------------------------------------------------------------------------
int32_t placeEvent(XEvent *event)
...
//------------------------------------------------------------------------
int32_t lux_menu_hide(ArgumentCount narg, Symbol ps[])
...
// if argument is -1, then hide all
{
 ...
 //------------------------------------------------------------------------
int32_t lux_menu_kill(ArgumentCount narg, Symbol ps[])
...
//------------------------------------------------------------------------
int32_t lux_check_menu(ArgumentCount narg, Symbol ps[])
...
//---------------------------------------------------------------------
int32_t lux_menu_read(ArgumentCount narg, Symbol ps[])
        ...
        Syntax:  menuread,menu#,item#,query,answer */
{
 ...
 //------------------------------------------------------------------------
int32_t lux_wait_for_menu(ArgumentCount narg, Symbol ps[])
// grabs pointer and waits for menu input
{
 ...
 //------------------------------------------------------------------------
int32_t lux_menu_pop(ArgumentCount narg, Symbol ps[])
...
{
  int32_t        i, direction, ascent, descent, menu_width = 0,
...
//------------------------------------------------------------------------
int32_t lux_menu(ArgumentCount narg, Symbol ps[])
/* try at menus.  Syntax:
...
*/
{
 int32_t        num, i, n_items, iq, x = 0, y = 0;
...
//------------------------------------------------------------------------
int32_t redefine_menu(ArgumentCount narg, Symbol ps[])
...
//------------------------------------------------------------------------
int32_t lux_menu_item(ArgumentCount narg, Symbol ps[])
   ...
   LS 29apr93 */
{
 ...
 //------------------------------------------------------------------------
int32_t define_menu(int32_t x, int32_t y, ArgumentCount narg, Symbol ps[])
...
#define X_WINDOW        256
#define X_MENU                512
static int32_t        XRegisteredWindow[MAXWINDOWS], XRegisteredMenu[MAXMENU];
...
static int32_t        N_XMask = sizeof(eventCode)/sizeof(int32_t);
int32_t        lux_event, eventSource;
extern int32_t        lux_button, root_x, root_y, xcoord, ycoord, last_wid,
                ht[], wd[];
extern float        xhair, yhair;
extern Window        win[];

int32_t lux_register_event(ArgumentCount narg, Symbol ps[])
...
#define XLOOP_MENU        -2
#define XLOOP_NONE        -3
int32_t lux_xloop(ArgumentCount narg, Symbol ps[])
...
{
  XEvent        event;
  int32_t        type, arg, i, j, mask;
  char        status = 0, buffer[4];
  KeySym        keysym;
  Window        w;
  extern int32_t        lux_keycode, lux_button, lux_keysym, lux_keystate;
...
// returns name of LUX X event
{
 ...
 //------------------------------------------------------------------------
int32_t lux_event_name(ArgumentCount narg, Symbol ps[])
{
  int32_t        type, result;
