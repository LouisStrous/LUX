/* File plots.c */
/* ANA routines dealing with line plots. */
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
#if HAVE_LIBX11
...
static char rcsid[] __attribute__ ((unused)) =
 "$Id: plots.c,v 4.0 2001/02/07 20:37:04 strous Exp $";
...
static	Int	xsym, ysym, xtitlesym, ytitlesym, titlesym, symStyle, nx, ny,
  exsym, eysym;
static	Int	nelem, isym, nbreak, *qi, huge = INT32_MAX, dq3, dq4,
...
Int fixPlotStyle(Int *symbol, Int *line)
...
{
  Int	ixlog, iylog, i, j;
  Float	x, y, stepxdvi, stepydvi, x2, y2;
  Float	xlabel, f;
  ...
  void	setupticks(Float *min, Float *max, Int ndlab, Int fstep, Int ilog,
		   ...
		   Float plim_max, Float *start, Float *step, Float *stepdvi);

  if (!plims[0] || !plims[1])	/* !XB or !XT nonzero -> need extremes */
    mm(xx, n, &xmax, &xmin);
  if (plims[0])
    xmin = plims[0];
  if (plims[1])
    ...
    xmax++;
  if (!plims[2] || !plims[3]) 	/* !YB or !YT nonzero -> need extremes */
    mm(yy, n, &ymax, &ymin);
  if (plims[2])
    ymin = plims[2];
  if (plims[3])
  ...
  iylog = ipltyp % 2;
  ixlog = (ipltyp/2) % 2;
  if (ixlog && xmin <= 0.0) {	/* use the smallest positive value */
    xmin = FLT_MAX;
    for (i = 0; i <n; i++)
      if (xx[i] < xmin && xx[i] > 0)
	xmin = xx[i];
  }
  if (iylog && ymin <= 0.0) {	/* use the smallest positive value */
    ymin = FLT_MAX;
    for (i = 0; i <n; i++)
      if (yy[i] < ymin && yy[i] > 0)
	ymin = yy[i];
  ...
  setupticks(&xmin, &xmax, ndlabx, fstepx, ixlog, irxf, ifzx, wxb, wxt,
	     plims[0], plims[1], &startx, &stepx, &stepxdvi);
  setupticks(&ymin, &ymax, ndlaby, fstepy, iylog, iryf, ifz, wyb, wyt,
	     plims[2], plims[3], &starty, &stepy, &stepydvi);
  ...
  *yy = y[0];
  ...
  if (symStyle != 20) {		/* not a bar graph */
    ...
    if (c != NONE_IN) {		/* the data point is in the plot window */
      tkplot(*xx, *yy, 0, 0);
      if (symStyle)
	tkplot(*xx, *yy, 0, symStyle); /* first symbol */
      if (ex) {			/* draw error bar */
	xe = x[0] - ex[0];	/* go left first */
	coordMap(&xe, NULL);
	if (noclipbars || xe >= wxb) {
	  tkplot(xe, yl - xerrs, 0, 0); /* we draw the left vertical bar */
	  tkplot(xe, yl + xerrs, 1, 0);
	  tkplot(xe, yl, 0, 0);
	} else
	  tkplot(wxb, yl, 0, 0); /* start drawing at plot edge */
	xe = x[0] + ex[dy];	/* then go right */
	coordMap(&xe, NULL);
	if (noclipbars || xe <= wxt) { 
	  tkplot(xe, yl, 1, 0);	/* draw the horizontal bar */
	  tkplot(xe, yl - xerrs, 0, 0); /* and the right vertical bar */
	  tkplot(xe, yl + xerrs, 1, 0);
	} else
	  tkplot(wxt, yl, 1, 0);
	tkplot(*xx, *yy, 0, 0);	/* and then return to the data point */
      }
      if (ey) {			/* draw error bar */
	ye = y[0] + ey[0];	/* go up first */
	coordMap(NULL, &ye);
	if (noclipbars || ye <= wyt) {
	  tkplot(xl - yerrs, ye, 0, 0); /* we draw the top horizontal bar */
	  tkplot(xl + yerrs, ye, 1, 0);
	  tkplot(xl, ye, 0, 0);
	} else
	  tkplot(xl, wyt, 0, 0);
	ye = y[0] - ey[dy];	/* then go down */
	coordMap(NULL, &ye);
	if (noclipbars || ye >= wyb) {
	  tkplot(xl, ye, 1, 0);	/* draw the vertical bar */
	  tkplot(xl - yerrs, ye, 0, 0); /* and the bottom horizontal bar */
	  tkplot(xl + yerrs, ye, 1, 0);
	} else
	  tkplot(xl, wyb, 1, 0);
	tkplot(*xx, *yy, 0, 0);	/* and then return to the data point */
    ...
    if (n > 1) {
      for (i = 1; i < n; i++) {	/* loop over (remaining) points */
	*xx = x[i];
	*yy = y[i];
	coordMap(xx, yy);
	c = clipToWindow(&xl, &yl, xx, yy, plotWindow);
...
Int ana_xymov(Int narg, Int ps[])			/* xymov routine */
 /* subroutine, call is xymov(x, y,mode, /boundingbox) */
{
