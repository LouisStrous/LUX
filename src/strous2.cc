...
// comparison function for use with lux_tolookup.  LS 9nov95
{
  ...
  case LUX_INT8:
    for (i = 0; i < multiCompNCoord; i++) {
      d.l = (int32_t) multiCompData.b[i1 + i*multiCompNPoints]
        - (int32_t) multiCompData.b[i2 + i*multiCompNPoints];
      if (d.l)
        return d.l;
    }
    break;
  ...
  case LUX_INT32:
    for (i = 0; i < multiCompNCoord; i++) {
      d.l = (int32_t) multiCompData.l[i1 + i*multiCompNPoints]
        - (int32_t) multiCompData.l[i2 + i*multiCompNPoints];
      if (d.l)
        return d.l;
    }
    break;
  case LUX_INT64:
    for (i = 0; i < multiCompNCoord; i++) {
      d.l = (int32_t) multiCompData.q[i1 + i*multiCompNPoints]
        - (int32_t) multiCompData.q[i2 + i*multiCompNPoints];
      if (d.l)
        return d.l;
    }
    break;
...
// 1D vector.  LS 9nov95 7may96 16oct97
{
  ...
  iq = *ps;                     // SRC
  multiCompData.l = (int32_t *) array_data(iq);
...
// auxilliary for lux_medianfilter
{
  ...
  switch (type)
  { case LUX_INT8:
      d1.b = src.b[*(int32_t *) x1];
      d2.b = src.b[*(int32_t *) x2];
    ...
    case LUX_INT64:
      d1.q = src.q[*(int32_t *) x1];
      d2.q = src.q[*(int32_t *) x2];
...
// LS 23oct98
{
  ...
  do {
    ...
    i++;
  } while (tmpinfo.advanceLoop(&tmpsrc) < tmpinfo.ndim);
...
// LS 31dec95 30jul97
{
  ...
  for (loop = 0; loop < srcinfo.naxes; loop++) {
    ...
    switch (type) {
      ...
      case LUX_INT64:
        do {
          value.q = bounds.max.q;       // initialize
          for (i = 0; i < ww; i++) { // do the left edge
            if (*src.q < value.q)
              value.q = *src.q;
            src.q += stride;
...
// LS 31dec95 30jul97
{
  ...
  for (loop = 0; loop < srcinfo.naxes; loop++) {
    ...
    switch (type) {
      ...
      case LUX_INT64:
        do {
          value.q = bounds.min.q;       // initialize
          for (i = 0; i < ww; i++) { // do the left edge
            if (*src.q > value.q)
              value.q = *src.q;
            src.q += stride;
    ...
    case LUX_INT8:
      do {
        ...
        for (j = 0; j < n; j++) {       // all directions
          ...
          srcl.b = src.b + k;
          srcr.b = src.b - k;
          bool ok = (sign && *src.b >= *srcl.b && *src.b > *srcr.b)
        ...
        if (degree)
          *trgt.l = ok_count;
          ...
          hit_locations.push_back(src.b - (uint8_t *) srcinfo.data0);
    ...
    case LUX_INT32:
      do {
        ...
        for (j = 0; j < n; j++) {       // all directions
          ...
          srcl.l = src.l + k;
          srcr.l = src.l - k;
          bool ok = (sign && *src.l >= *srcl.l && *src.l > *srcr.l)
        ...
        if (degree)
          *trgt.l = ok_count;
          ...
          hit_locations.push_back(src.l - (int32_t *) srcinfo.data0);
        done = degree? (trgtinfo.advanceLoop(&trgt),
                        srcinfo.advanceLoop(&src)):
          srcinfo.advanceLoop(&src);
      } while (done < srcinfo.ndim);
    ...
    case LUX_INT64:
      do {
        ...
        for (j = 0; j < n; j++) {       // all directions
          ...
          srcl.q = src.q + k;
          srcr.q = src.q - k;
          bool ok = (sign && *src.q >= *srcl.q && *src.q > *srcr.q)
        ...
        if (degree)
          *trgt.q = ok_count;
          ...
          hit_locations.push_back(src.q - (int64_t *) srcinfo.data0);
        done = degree? (trgtinfo.advanceLoop(&trgt),
                        srcinfo.advanceLoop(&src)):
          srcinfo.advanceLoop(&src);
      } while (done < srcinfo.ndim);
...
*/
{
  ...
  case LUX_INT64:
    ...
    v.q = *data.q + 1;
    while (nData--) {           // all data points
      if (*data.q != v.q) { // we added this check because we
        // hope there is coherence in the data so that the next data
        // value has a fair chance of being equal to the previous one.
        // In that case we can skip the binary search.
        low = 0;
