...
int32_t segment_2d(ArgumentCount narg, Symbol ps[])
      ...
      case LUX_INT64:
        while (ny--) {          // all rows except for bottom one
          *trgt.i32++ = 0;
          ...
          while (n--) {
            value.i64 = *src.i64 * 2;
            *trgt.i32++ = (src.i64[-1] + src.i64[1] < value.i64
                         && src.i64[nx] + src.i64[-nx] < value.i64
                         && src.i64[1 + nx] + src.i64[-1 - nx] < value.i64
                         && src.i64[1 - nx] + src.i64[-1 + nx] < value.i64);
            src.i64++;
          }
          *trgt.i32++ = 0;
...
//-------------------------------------------------------------------------
int32_t segment_general(ArgumentCount narg, Symbol ps[])
      ...
      case LUX_INT64:
        do {
          ...
          for (j = 0; j < n; j++) {     // all directions
            ...
            srcl.i64 = src.i64 + k;
            srcr.i64 = src.i64 - k;
            if (value.i64 <= *srcl.i64 + *srcr.i64)
              break;
          }
          *trgt.i32 = (j == n);
        } while (trgtinfo.advanceLoop(&trgt.ui8),
        ...
        case LUX_INT64:
          do {
            ...
            for (j = 0; j < n; j++) { // all directions
              ...
              srcl.i64 = src.i64 + k;
              srcr.i64 = src.i64 - k;
              ok = (sign && value.i64 > *srcl.i64 + *srcr.i64)
                || (!sign && value.i64 < *srcl.i64 + *srcr.i64);
              if (degree)
                nok += ok;
              else if (!ok) {
                nok = 0;
...
//-------------------------------------------------------------------------
int32_t lux_segment(ArgumentCount narg, Symbol ps[])
{
...
int32_t lux_segment_dir(ArgumentCount narg, Symbol ps[])
      ...
      case LUX_INT64:
        while (ny--) {          // all rows except for bottom one
          *trgt.i32++ = 0;
          ...
          n = nx - 2;
          while (n--) {
            value.i64 = *src.i64 * (short) 2;
            a = *angle++ + DEG22_5;
            s = sin(a);
            c = cos(a);
            class_id = ((s*c > 0) << 1) + (s*c*(2*c*c - 1) > 0);
            *trgt.i32++ = (src.i64[off[class_id]] + src.i64[-off[class_id]] < value.i64);
            src.i64++;
          }
          *trgt.i32++ = 0;
          src.i64++;
...
//-------------------------------------------------------------------------
int32_t lux_max_dir(ArgumentCount narg, Symbol ps[])
        ...
        while (ny--) {          // all rows except for bottom one
          *trgt.ui8++ = 0;
          ...
          while (n--) {
            ...
            c = cos(a);
            class_id = ((s*c > 0) << 1) + (s*c*(2*c*c - 1) > 0);
            *trgt.ui8++ = (value.i64 > src.i64[off[class_id]]
                         && value.i64 > src.i64[-off[class_id]]);
            src.i64++;
          }
          *trgt.ui8++ = 0;
          src.i64++;
...
#define MARKED          -2
#define EDGEMARKED      -3
int32_t area_2d(ArgumentCount narg, Symbol ps[])
/* AREA,image
...
//-------------------------------------------------------------------------
int32_t area_general(ArgumentCount narg, Symbol ps[], int32_t isFunction)
...
//-------------------------------------------------------------------------
int32_t lux_area(ArgumentCount narg, Symbol ps[])
...
//----------------------------------------------------------------------
#define SEEK_MAXIMUM    (LUX_DOUBLE + 1)
int32_t area2_2d(ArgumentCount narg, Symbol ps[])
      ...
      for (direction = 0; direction < N_DIRECTIONS; direction++) {
        ...
        switch (maximum + type) {
          ...
          case SEEK_MAXIMUM + LUX_INT64:
            if (dataptr.i64[offset[direction]] > *dataptr2.i64) {
              j = direction;
              dataptr2.i64 = dataptr.i64 + offset[direction];
            }
            break;
        ...
        for (direction = 0; direction < N_DIRECTIONS; direction++) {
          ...
          switch (maximum + type) {
            ...
            case SEEK_MAXIMUM + LUX_INT32:
              ...
              break;
            case SEEK_MAXIMUM + LUX_INT64:
              if (dataptr.i64[offset[direction]] >= *dataptr.i64)
                continue;
              break;
            case SEEK_MAXIMUM + LUX_FLOAT:
...
//----------------------------------------------------------------------
int32_t area2_general(ArgumentCount narg, Symbol ps[])
...
//----------------------------------------------------------------------
int32_t lux_area2(ArgumentCount narg, Symbol ps[])
{
  if (!symbolIsNumericalArray(ps[0]) || array_type(ps[0]) != LUX_INT32)
    return luxerror("Need LONG array", ps[0]);
  if (!symbolIsNumericalArray(ps[1]))
...
//----------------------------------------------------------------------
int32_t lux_basin(ArgumentCount narg, Symbol ps[])
...
//----------------------------------------------------------------------
int32_t lux_basin2(ArgumentCount narg, Symbol ps[])
  ...
  switch (symbol_type(ps[0]) | sign) {
    ...
    case LUX_INT64:
      ...
      do {
        ...
        loc[2] = loc[1];
        loc[1] = loc[0];
        min[0].i64 = bounds.max.i64;
        mini++;
        if (!srcinfo.coords[0]) {       // at left edge
          src.i64--;
          min[2].i64 = min[1].i64 = bounds.max.i64;
          mini = 3;
          edge = 0;
          for (i = 1; i < srcinfo.ndim; i++)
            if (!srcinfo.coords[i]
          ...
          if (edge)
            for (i = 0; i < n; i++) { // all directions
              ...
              if (j == srcinfo.ndim - 1) {      // not across any other edge
                if (src.i64[offsets[i]] < min[0].i64) { // update
                  min[0].i64 = src.i64[offsets[i]];
                  loc[0] = i;
                }
              }
            } else for (i = 0; i < n; i++) {
          ...
          if (mini < 3) {       // previous lowest is now in 1 or 2
            if (min[0].i64 < min[mini].i64)
              mini = 0;
          } else {              // previous lowest is too far away: find new
            if (min[0].i64 < min[1].i64) {
              if (min[0].i64 < min[2].i64)
                mini = 0;
              else
                mini = 2;
            } else {
              if (min[1].i64 < min[2].i64)
                mini = 1;
              else
                mini = 2;
            }
          }
          src.i64++;              // back to current position
          min[2].i64 = min[1].i64;
          min[1].i64 = min[0].i64;
          loc[2] = loc[1];
          loc[1] = loc[0];
    ...
    case LUX_INT64 | 0x20:
      ...
      do {
        ...
        loc[2] = loc[1];
        loc[1] = loc[0];
        max[0].i64 = bounds.min.i64;
        maxi++;
        if (!srcinfo.coords[0]) {       // at left edge
          src.i64--;
          max[2].i64 = max[1].i64 = bounds.min.i64;
          maxi = 3;
          edge = 0;
          for (i = 1; i < srcinfo.ndim; i++)
            if (!srcinfo.coords[i]
          ...
          if (edge)
            for (i = 0; i < n; i++) { // all directions
              ...
              if (j == srcinfo.ndim - 1) {      // not across any other edge
                if (src.i64[offsets[i]] > max[0].i64) { // update
                  max[0].i64 = src.i64[offsets[i]];
                  loc[0] = i;
                }
              }
            } else for (i = 0; i < n; i++) {
          ...
          if (maxi < 3) {       // previous highest is now in 1 or 2
            if (max[0].i64 > max[maxi].i64)
              maxi = 0;
          } else { // previous highest is too far away: find new
            if (max[0].i64 > max[1].i64) {
              if (max[0].i64 > max[2].i64)
                maxi = 0;
              else
                maxi = 2;
            } else {
              if (max[1].i64 > max[2].i64)
                maxi = 1;
              else
                maxi = 2;
            }
...
//----------------------------------------------------------------------
int32_t lux_extreme_general(ArgumentCount narg, Symbol ps[])
            ...
            for (i = i1; i < i2; i++) { // center data points
              *trgt.ui8 = 0;
              for (j = 0; j < n; j++) {         // all directions
                ...
                srcl.i64 = src.i64 + k;
                srcr.i64 = src.i64 - k;
                *trgt.ui8 += (*src.i64 > *srcl.i64 && *src.i64 > *srcr.i64);
              }
              src.i64++;
              trgt.ui8++;
            }
              ...
              for (i = i1; i < i2; i++) { // center data points
                *trgt.ui8 = 0;
                for (j = 0; j < n; j++) { // all directions
                  ...
                  srcl.i64 = src.i64 + k;
                  srcr.i64 = src.i64 - k;
                  *trgt.ui8 += ((sign && *src.i64 > *srcl.i64 + *t.i64
                               && *src.i64 > *srcr.i64 + *t.i64)
                              || (!sign && *src.i64 < *srcl.i64 - *t.i64
                                  && *src.i64 < *srcr.i64 - *t.i64));
                }
                trgt.ui8++;
                src.i64++;
...
//----------------------------------------------------------------------
int32_t lux_area_connect(ArgumentCount narg, Symbol ps[])
